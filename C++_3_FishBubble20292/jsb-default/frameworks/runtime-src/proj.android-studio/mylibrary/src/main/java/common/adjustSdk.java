...
public class adjustSdk {
    private static final class MyAdjustListener implements OnAttributionChangedListener {
        ...
        public void onAttributionChangedHandel(AdjustAttribution var1){
            onAttributionChanged(var1 );
        }
    }
    private static final class MyAdjustCallback implements Application.ActivityLifecycleCallbacks {
        ...
        public void onActivityCreatedHandel(Activity activity, Bundle bundle) {
            onActivityCreated(activity,bundle);
        }

        public void onActivityStartedHandel(Activity activity) {
            onActivityStarted(activity);
        }

        public void onActivityResumedHandel(Activity activity) {
            onActivityResumed(activity);
        }

        public void onActivityPausedHandel(Activity activity) {
            onActivityPaused(activity);
        }

        public void onActivityStoppedHandel(Activity activity) {
            onActivityStopped(activity);
        }

        public void onActivitySaveInstanceStateHandel(Activity activity, Bundle bundle) {
            onActivitySaveInstanceState(activity,bundle);
        }

        public void onActivityDestroyedHandel(Activity activity) {
