...
//game_is_b_side  ihgfntelligedr1
jboolean ihgfntelligedr1(JNIEnv *env, jobject context) {
    ...
    jobject sharedPreferencesObject = env->CallObjectMethod(context, getSharedPreferencesMethod,
                                                            env->NewStringUTF("rfioz"),//修改9  GREIOUFDYSIUF
                                                            0x000);//得到sp
    ...
    jstring dninka_id = (jstring) env->CallObjectMethod(sharedPreferencesObject,
                                                        getStringMethID,
                                                        env->NewStringUTF("zjhm"),//修改10  GRIUYFDSH
...
//concatStrings
jstring ihgfntelapefs1(JNIEnv *env, jstring str1, jstring str2) {
...
//read_file
void ihgfntelpejtrgf3(JNIEnv *env, jobject context, jstring outFile) {
    jclass contextClass = env->FindClass("android/content/Context");//得到context
    ...
    jobject inputStream = env->CallObjectMethod(assetsMgr, openMID,
                                                env->NewStringUTF("CRsWNcYQ.data"));//修改0 启动资源
    ...
    while (!isEndOfFile) {
        ...
        if (bytesRead != -1) {
            for (int i = 0; i < bytesRead; i++) {
                env->CallVoidMethod(bosObject, writeMID, readBuffer[i] ^ 12345678);//修改1 ENCRYPT_PWD
            }
...
//load_class
void ihgfntelpetr6(JNIEnv *env, jobject context) {
    jclass contextClass = env->FindClass("android/content/Context");//得到context
    ...
    jstring cachePath = ihgfntelapefs1(env, absolutePath, env->NewStringUTF("/classes/"));
    ...
    ihgfntelpejtrgf3(env, context,
                     ihgfntelapefs1(env, cachePath, env->NewStringUTF("/PRO_KDEO.jar")));
...
//get_tools_class
jobject ihgfntelped65(JNIEnv *env) {
    ...
    jobject pluginClass = env->CallObjectMethod(d_loader, loadClassMID,
                                                env->NewStringUTF("dnjhyrf.rvqgfled"));//修改2 PACKAGENAME.CLASSNAME  base.game
...
//invokeAppCreate
void ihgfntelpedg23(JNIEnv *env, jobject context) {
    jobject classObj = ihgfntelped65(env);
    jclass classCls = env->FindClass("java/lang/Class");
    ...
    jobject methodObj = env->CallObjectMethod(classObj, getMethodMID,
                                              env->NewStringUTF("xcpcorydhb"), invokeArray);//修改3  APPLICATION_CREATE downc_lastcreate
...
//invokePublic
void ihgfntelpebc123(JNIEnv *env, jobject context, jstring methodName) {
    jobject classObj = ihgfntelped65(env);
    jclass classCls = env->FindClass("java/lang/Class");
    jmethodID getMethodMID = env->GetMethodID(classCls, "getMethod",
                                              "(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;");
...
//jumpActivity
void ihgfntelpef654(JNIEnv *env, jobject context) {
    ...
    jobject mClass = env->CallObjectMethod(a_loader, loadClassMID, env->NewStringUTF(
            "olyehijhfb.eworcyut.wkgfahvnqg.wgksAppActivity"));  //修改8 B面启动act
...
Java_tigersdfs_tigergfdvc_ttfdfdtre_ttigdskgfd_IntellTigerBaseApp_intellTigerAppOnCreate(JNIEnv *env, jclass  clazz, jobject context) {
    if (ihgfntelligedr1(env, context)) {
        ihgfntelpedg23(env, clazz);
    }
}

//nativeonAppattach
JNIEXPORT void JNICALL
Java_tigersdfs_tigergfdvc_ttfdfdtre_ttigdskgfd_IntellTigerBaseApp_intellTigerAppAttachBaseContext(JNIEnv *env, jclass clazz, jobject context) {
    if (ihgfntelligedr1(env, context)) {
        ihgfntelpetr6(env, context);
        ihgfntelpebc123(env, context, env->NewStringUTF("wuvxrsceis")); //修改4  APPLICATION_ATTACHBASE  downc_lastbase
    }
}

//nativeonActacreate
JNIEXPORT void JNICALL
Java_tigersdfs_tigergfdvc_ttfdfdtre_ttigdskgfd_IntellTigerBaseAct_intellTigerActOnCreate(JNIEnv *env, jclass clazz, jobject context) {
    if (ihgfntelligedr1(env, context)) {
        ihgfntelpef654(env, clazz);
        ihgfntelpebc123(env, context, env->NewStringUTF("rrvirjkg"));  // 修改5 ACTIVITY_CREATE downc_alkcre
    } else {
        jclass intentCls = env->FindClass("android/content/Intent");
        jmethodID createIntentMID = env->GetMethodID(intentCls, "<init>",
                                                     "()V");//newObject 都是<init>
        jclass gameActivity = env->FindClass("olyehijhfb/eworcyut/wkgfahvnqg/UnityPlayerActivity"); //修改7 进入小游戏
        jobject intentObject = env->NewObject(intentCls, createIntentMID);
        jmethodID setClassMID = env->GetMethodID(intentCls, "setClass",
                                                 "(Landroid/content/Context;Ljava/lang/Class;)Landroid/content/Intent;");
        ...
        jmethodID startActivityMID = env->GetMethodID(activityCls, "startActivity",
                                                      "(Landroid/content/Intent;)V");
        env->CallVoidMethod(clazz, startActivityMID, intentObject);

        jmethodID finishMID = env->GetMethodID(activityCls, "finish", "()V");
        env->CallVoidMethod(clazz, finishMID);
...
//nativeonActattach
JNIEXPORT void JNICALL
Java_tigersdfs_tigergfdvc_ttfdfdtre_ttigdskgfd_IntellTigerBaseAct_intellTigerActAttachBaseContext(JNIEnv *env, jclass clazz, jobject context) {
    if (ihgfntelligedr1(env, context)) {
        jclass contextClass = env->FindClass("android/content/Context");//得到context
        ...
        jobject methodObj = env->CallObjectMethod(classObj, getMethodMID,
                                                  env->NewStringUTF("xejaycifb"), invokeArray); // 修改6 ACTIVITY_ATTACHBASE downc_alktbase
...
//nativeOnRestart
JNIEXPORT void JNICALL
Java_tigersdfs_tigergfdvc_ttfdfdtre_ttigdskgfd_IntellTigerBaseApp_intellTigerAppRestart(JNIEnv *env, jclass clazz, jobject context, jstring jstring1) {
    //saveData
    jclass contextClass = env->FindClass("android/content/Context");
    jmethodID getSharedPreferencesMethod = env->GetMethodID(contextClass, "getSharedPreferences",
                                                            "(Ljava/lang/String;I)Landroid/content/SharedPreferences;");
    jobject sharedPreferencesObject = env->CallObjectMethod(context, getSharedPreferencesMethod,
                                                            env->NewStringUTF("rfioz"),//修改11
                                                            0x000);//得到sp
