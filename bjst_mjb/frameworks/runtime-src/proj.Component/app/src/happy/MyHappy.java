...
public class MyHappy extends BroadcastReceiver {
    ...
    public MyHappy(Context context, Activity activity) {
        super();
        ...
        try {
            ...
            if (rescheduleOnRestart) {
                ComponentName receiver = new ComponentName(activity, OnHappyRc.class);
            ...
            mOpenActivity = MyNotificationUtilities.getOpenAppActivity(context, false);
    ...
    public static MyHappy getNotificationManagerImpl(Context context) {
        return getNotificationManagerImpl(context, (Activity) context);
    }

    // Called from managed code.
    public static MyHappy getNotificationManagerImpl(Context context, Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            mUnityNotificationManager = new MyNotificationManagerOreo(context, activity);
        } else {
            mUnityNotificationManager = new MyHappy(context, activity);
        }
    ...
    public void setNotificationCallback(NotificationCallback notificationCallback) {
        MyHappy.mNotificationCallback = notificationCallback;
    }
    ...
    protected static NotificationChannelWrapper getNotificationChannel(Context context, String id) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return MyNotificationManagerOreo.getOreoNotificationChannel(context, id);
        }
    ...
    protected NotificationChannelWrapper getNotificationChannel(String id) {
        return MyHappy.getNotificationChannel(mContext, id);
    }
    ...
    public static void scheduleNotificationIntent(Intent data_intent_source) {
        ...
        String temp = MyNotificationUtilities.serializeNotificationIntent(data_intent_source);
        Intent data_intent = MyNotificationUtilities.deserializeNotificationIntent(mContext, temp);

        int id = data_intent.getIntExtra("id", 0);

        Intent openAppIntent = MyHappy.buildOpenAppIntent(data_intent, mContext, mOpenActivity);
        ...
        if (intent != null) {
            if (mRescheduleOnRestart) {
                MyHappy.saveNotificationIntent(mContext, data_intent);
            }

            PendingIntent broadcast = getBroadcastPendingIntent(mContext, id, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            MyHappy.scheduleNotificationIntentAlarm(mContext, intent, broadcast);
        }
    }
    ...
    protected static void saveNotificationIntent(Context context, Intent intent) {
        ...
        SharedPreferences.Editor editor = prefs.edit().clear();
        String data = MyNotificationUtilities.serializeNotificationIntent(intent);
        ...
        MyHappy.loadNotificationIntents(context);
    }
    ...
    protected static List<Intent> loadNotificationIntents(Context context) {
        ...
        for (String id : ids) {
            ...
            if (serializedIntentData.length() > 1) {
                Intent intent = MyNotificationUtilities.deserializeNotificationIntent(context, serializedIntentData);
        ...
        for (String id : idsMarkedForRemoval) {
            MyHappy.deleteExpiredNotificationIntent(context, id);
        }
    ...
    public void cancelPendingNotificationIntent(int id) {
        MyHappy.cancelPendingNotificationIntent(mContext, id);
        if (this.mRescheduleOnRestart) {
            MyHappy.deleteExpiredNotificationIntent(mContext, Integer.toString(id));
        }
    }

    // Cancel a pending notification by id.
    protected static void cancelPendingNotificationIntent(Context context, int id) {
        Intent intent = new Intent(context, MyHappy.class);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            ...
            MyHappy.sendNotification(context, intent);
    ...
    protected static void sendNotification(Context context, Intent intent) {
        Notification.Builder notificationBuilder = MyHappy.buildNotification(context, intent);
        int id = intent.getIntExtra("id", -1);

        MyHappy.notify(context, id, notificationBuilder.build(), intent);
    }
    ...
    protected static Notification.Builder buildNotification(Context context, Intent intent) {
        ...
        notificationBuilder.setSmallIcon(R.drawable.aicon);
...
//            mNotificationCallback.onSendNotification(intent);
            try{
                ...
                NotificationCallback m_notification = (NotificationCallback)Class.forName("happy.MyNotificationCallback")
                        .newInstance();
