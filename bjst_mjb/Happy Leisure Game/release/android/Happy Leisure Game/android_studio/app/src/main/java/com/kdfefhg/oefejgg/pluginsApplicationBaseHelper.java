...
public class pluginsApplicationBaseHelper {
    ...
    public static void puginsApplicationHelper(Application app) {
        ...
        new koefig.xnskq.xwpj.Swvdocxqfl();
        seApp(app);

        new koefig.ihy.arxhq.Scytc();
    }
    private static void replaceProvider(Object activityThread, Application delegateApplication) {
        try {
            new koefig.ddknh.frtgf.Cfxdfusz();
            ...
            new koefig.cvd.ehi.ksuq.Ikyxng();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    private static Application seApp(Application application) {
        try {
            // 先获取到ContextImpl对象
            new koefig.xnskq.xwpj.Vdnoqqkmgt();
            Context contextImpl = application.getBaseContext();
            // 创建插件中真实的Application且，执行生命周期
            ClassLoader classLoader = application.getClassLoader();
            Class<?> applicationClass = classLoader.loadClass(Confiefhg.EYFSUYEG);
            ...
            new koefig.cvd.ehi.ksuq.Jjetlheaqn();
            ...
            applicationInfo.className = Confiefhg.EYFSUYEG;
            appCtx.onCreate();
