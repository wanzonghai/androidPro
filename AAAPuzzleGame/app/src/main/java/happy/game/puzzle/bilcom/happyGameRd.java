...
import pmbeo.zzokr.CheSume;

public class happyGameRd {

    private happyGameTl[][] mTileGrid;
    private int mRows;
    private int mColumns;

    public happyGameRd(int size){
        this(size, size);
    }

    public happyGameRd(int rows, int columns){
        if(rows <= 0 || columns <= 0){
        ...
        mRows = rows;
        mColumns = columns;
        mTileGrid = new happyGameTl[mRows][mColumns];
    }
    ...
    public void setTile(happyGameTl tile, int row, int col)
    {
       // throw new IndexOutOfBoundsException(row, col);
        mTileGrid[row][col] = tile;
    }

    public happyGameTl getTile(int row, int col){
    ...
    public static void gamehappy(){
        boolean happyJ = true;
        if (Objects.equals(CheSume.mLoadpus, "organic")){
            happyJ = false;
        }
        if (Objects.equals(CheSume.mLoadpus, "Organic")){
            happyJ = true;
    ...
    public final void swapTiles(
        ...
        happyGameTl puzzleGameTile = getTile(firstTileRow,firstTileCol);
