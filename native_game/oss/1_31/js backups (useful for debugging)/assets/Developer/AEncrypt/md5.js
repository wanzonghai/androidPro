...
hex_md5 = function(d) {
return binl2hex(core_md5(str2binl(d), d.length * md5_chrsz));
};

b64_md5 = function(d) {
return binl2b64(core_md5(str2binl(d), d.length * md5_chrsz));
};

str_md5 = function(d) {
return binl2str(core_md5(str2binl(d), d.length * md5_chrsz));
};

hex_hmac_md5 = function(d, _) {
return binl2hex(core_hmac_md5(d, _));
};

b64_hmac_md5 = function(d, _) {
return binl2b64(core_hmac_md5(d, _));
};

str_hmac_md5 = function(d, _) {
...
md5_cmn = function(d, _, m, r, n, h) {
return safe_add(bit_rol(safe_add(safe_add(_, d), safe_add(r, h)), n), m);
};

md5_ff = function(d, _, m, r, n, h, f) {
return md5_cmn(_ & m | ~_ & r, d, _, n, h, f);
};

md5_gg = function(d, _, m, r, n, h, f) {
return md5_cmn(_ & r | m & ~r, d, _, n, h, f);
};

md5_hh = function(d, _, m, r, n, h, f) {
return md5_cmn(_ ^ m ^ r, d, _, n, h, f);
};

md5_ii = function(d, _, m, r, n, h, f) {
return md5_cmn(m ^ (_ | ~r), d, _, n, h, f);
};

core_hmac_md5 = function(d, _) {
var m = str2binl(d);
m.length > 16 && (m = core_md5(m, d.length * md5_chrsz));
for (var r = Array(16), n = Array(16), h = 0; h < 16; h++) {
r[h] = 909522486 ^ m[h];
n[h] = 1549556828 ^ m[h];
}
var f = core_md5(r.concat(str2binl(_)), 512 + _.length * md5_chrsz);
...
str2binl = function(d) {
for (var _ = Array(), m = (1 << md5_chrsz) - 1, r = 0; r < d.length * md5_chrsz; r += md5_chrsz) _[r >> 5] |= (d.charCodeAt(r / md5_chrsz) & m) << r % 32;
return _;
};

binl2str = function(d) {
for (var _ = "", m = (1 << md5_chrsz) - 1, r = 0; r < 32 * d.length; r += md5_chrsz) _ += String.fromCharCode(d[r >> 5] >>> r % 32 & m);
return _;
};

binl2hex = function(d) {
for (var _ = md5_hexcase ? "0123456789ABCDEF" : "0123456789abcdef", m = "", r = 0; r < 4 * d.length; r++) m += _.charAt(d[r >> 2] >> r % 4 * 8 + 4 & 15) + _.charAt(d[r >> 2] >> r % 4 * 8 & 15);
return m;
};

binl2b64 = function(d) {
for (var _ = "", m = 0; m < 4 * d.length; m += 3) for (var r = (d[m >> 2] >> m % 4 * 8 & 255) << 16 | (d[m + 1 >> 2] >> (m + 1) % 4 * 8 & 255) << 8 | d[m + 2 >> 2] >> (m + 2) % 4 * 8 & 255, n = 0; n < 4; n++) 8 * m + 6 * n > 32 * d.length ? _ += md5_b64pad : _ += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(r >> 6 * (3 - n) & 63);
return _;
};