...
--HTTP获取json
function appdf.onHttpJsonTable(url,methon,params,callback,pResponseType)
	print("appdf.onHttpJsonTable:"..url)
	local xhr = cc.XMLHttpRequest:new()
	xhr.responseType = pResponseType or cc.XMLHTTPREQUEST_RESPONSE_JSON
	...
	local function onJsonTable()
	    ...
	    if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
	   		...
	   		if response then
				if xhr.responseType == 0 then
					datatable = response
				elseif xhr.responseType == 4 then
					ok, datatable = pcall(function()
						return cjson.decode(response)
					end)
					if not ok then
						 print("onHttpJsonTable_cjson_error,url = ",url)
						 datatable = nil
					end
				end
		    end
	    else
	    	print("onJsonTable http fail readyState:"..xhr.readyState.."#status:"..xhr.status)
	    end
	    if type(callback) == "function" then
	    	callback(datatable,response)
	    end	    
	end
	xhr:registerScriptHandler(onJsonTable)
    if false then
	    if not bPost then
	    	xhr:send()
	    else
...
function appdf.DownloadZip(uri,savepath,progressCallback,resultCallback)
	local PreProcess = 0
	...
	--注册下载失败回调函数
	downloader:setOnTaskError(function(task)
		downloader:__gc()		
	...
	downloader:setOnTaskProgress(function(task, bytesReceived, totalBytesReceived, totalBytesExpected)		
		local progress = totalBytesReceived * 100 / totalBytesExpected 
		if progress > 0 and progressCallback then
			if progress - PreProcess > 1 or progress then
				PreProcess = progress
				progressCallback(progress)
			end
		end
	end)
	local id = appdf.TaskID;
	appdf.TaskID = appdf.TaskID + 1;

	local task = downloader:createDownloadFileTask(uri, savepath,"Task_"..tostring(id))
end
...
function appdf.IPV6ToIPV4(ipAddress)
    local result = ipAddress
    local parts = {}    
    -- 将IPv6地址分割为8个16位部分
    ...
    -- 检查IPv4映射规则
    if #parts == 8 and parts[7] and string.len(parts[7])==4 and parts[8] and string.len(parts[8])==4 then  
        local s1 = tonumber(string.sub(parts[7],1,2),16)
        local s2 = tonumber(string.sub(parts[7],3,4),16)
        local s3 = tonumber(string.sub(parts[8],1,2),16)
        local s4 = tonumber(string.sub(parts[8],3,4),16)
        result = s1.."."..s2.."."..s3.."."..s4
    end
    return result
end
...
--获取ip
function appdf.onGetClientIp(cmdUrl)
	print("Start call appdf.onGetClientIp(cmdUrl),cmdUrl=",cmdUrl)
	local result = "127.0.0.1"
	if g_TargetPlatform == cc.PLATFORM_OS_IPHONE or g_TargetPlatform == cc.PLATFORM_OS_IPAD then
		...
		if IP_OPTIMIZE_2 then
			local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS_IOS,"CCGetClientIp",{cmdUrl = cmdUrl})
			if ok and ret and ret~="" then result = ret end
		else
			local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS_IOS,"CCGetClientIp")
			if ok and ret and ret~="" then result = ret end
		end
	else
		if IP_OPTIMIZE_2 then
			result = CCGetClientIp(cmdUrl) --win android通过popen获取,默认为127.0.0.1	
		else
			result = CCGetClientIp() --win android通过popen获取,默认为127.0.0.1
		end
		
	end
	print("Over call appdf.onGetClientIp(cmdUrl),result=",result)
	return result
end
