...
function GameLogic.GET_POKER_VALUE()
...
function GameLogic:storeTrumpCardData(_cardData)
	...
	local cardValue = GameLogic.POKER_VALUE[_cardData]
	local newIndex = 0
...
function GameLogic:sortCardIndex(_cardData)
	...
	for i, v in ipairs(filterData) do
		local color = colorArray[CARD_COLOR.CARD_COLOR[v] + 1]
		local realValue = GameLogic.POKER_VALUE[v]
