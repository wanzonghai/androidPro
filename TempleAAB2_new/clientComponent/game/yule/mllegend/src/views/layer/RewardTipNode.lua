-- 大奖提示界面
--local DialogBase = appdf.req("game.yule.jxlw.src.views.layer.DialogBase")
...
function RewardTipNode:ctor(_data)
    ...
    self.lGetScoreStep = _data._lGetScoreStep
    self.m_callBack = _data._callBack
    self._auto = _data._auto
...
function RewardTipNode:playEffect(sound)
    if GlobalUserItem.bSoundAble then
		return AudioEngine.playEffect(sound)
	end
end

function RewardTipNode:createAnimate(_rateNum)
    --粒子
    local particleFile = "BigWin/za_lb_jbfzmpdl"
	local particleAniName = "a1"--string.format("%s_start", aniNamePrefix)

    local sound = "sound_res/YOU_WIN.mp3"
    
    local aniNamePrefix = "a1"
    local spineFile = "BigWin/sb_lbjs_YOU_WIN/sb_lbjs_MEGAWIN"
    if _rateNum >= self.lGetScoreStep[4] then
        spineFile = "BigWin/sb_lbjs_SUPER_WIN/sb_lbjs_MEGAWIN"
    	aniNamePrefix = "a1"
        particleAniName = "a3"
        sound = "sound_res/SUPER_WIN.mp3"
    elseif _rateNum >= self.lGetScoreStep[3] then
        spineFile = "BigWin/sb_lbjs_MEGAWIN/sb_lbjs_MEGAWIN"
    	aniNamePrefix = "a1"
        particleAniName = "a2"
        sound = "sound_res/MEGA_WIN.mp3"
    elseif _rateNum >= self.lGetScoreStep[2] then
        spineFile = "BigWin/sb_lbjs_BIGWIN/sb_lbjs_BIGWIN"
    	aniNamePrefix = "a1"
        particleAniName = "a1"
        sound = "sound_res/BIG_WIN.mp3"
    end

    if _rateNum >= self.lGetScoreStep[2] then
    ...
    self.sound_handle = self:playEffect(sound)
