...
function CannonLayer:showCannonByChair( chair , wChairID)
        ...
        local add = ccui.Button:create("game_res/add.png","","")
                ...
                add:addTouchEventListener(function( sender , eventType )
						if GlobalUserItem.LaspPayTime and GlobalUserItem.LaspPayTime <= 0 then
							self.parent:showTipsChangeNote(function()
								-- body
								self.parent:onClickExitGame(true)
							end)
							local _tips_change_note = self.parent:getChildByName("_tips_change_note")
							if _tips_change_note ~= nil then
								_tips_change_note:updateLayer()
							end
							return
						end
	...
	local MinShoot = self._dataModel.m_secene.MinShoot or 5
	local labelMutiple = cc.LabelBMFont:create(g_format:formatNumber(MinShoot, g_format.fType.standard), "game_res/mutipleNum.fnt")
