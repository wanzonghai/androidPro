...
function Cannon:onExit( )
	...
	if nil ~= self._queryDialog then
		self._queryDialog:removeFromParent(true)
...
function Cannon:autoUpdate(dt)
	...
	if score < self.parent.parent.CurrShoot[1][self.m_nChairID+1] then
		...
		if nil == self._queryDialog then
			...
			if self.parent.parent.showJumpVipDialog then
				self.parent.parent:showJumpVipDialog(function()
					-- body
					self.parent.parent:onClickExitGame(true)
				end)
