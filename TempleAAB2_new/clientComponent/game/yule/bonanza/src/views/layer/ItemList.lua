...
function ItemList:onStart(callback)
    ...
    for i = 1, GameLogic.ITEM_Y_COUNT do
        ...
        item:show()
        local move = cc.EaseSineIn:create(cc.MoveBy:create(i * 0.1, cc.p(0, -GameLogic.TOTAL_HEIGHT)))
...
function ItemList:onEnd()
    ...
    for i = 1, GameLogic.ITEM_Y_COUNT do
        ...
        local move = cc.EaseSineOut:create(cc.MoveTo:create(i * 0.146, cc.p(posX, posY)))
        item:runRotate(0.2)
        item:show()
        local _time = i * 0
        item:stopAllActions()
...
function ItemList:getItemPosition(_index)
    ...
    local posX = 16 + GameLogic.ITEM_WIDTH * 0.5
    local posY = 4 + GameLogic.ITEM_HEIGHT * (_index - 0.5) --min y is GameLogic.ITEM_HEIGHT * 0.5
    return posX, posY
end
