...
function GameLayer:onEventGameSceneFree(buffer)
    ...
    GameLogic.Reward_Scope.superbig = buffer:readscore(int64):getvalue() or GameLogic.Reward_Scope.superbig
...
function GameLayer:onNormalStartBtnClick(callBack)
    tlog('GameLayer:onNormalStartBtnClick ', self.m_cbGameMode)
    --等待状态
    if self.m_cbGameMode == GAME_STATE.GAME_STATE_WAITTING then
        self:GameStart(callBack)
    else
