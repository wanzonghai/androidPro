...
function SubLayerJump:onAddEventListen()
   ...
   G_event:AddNotifyEvent(G_eventDef.UI_JUMP_VIP_DIALOG,handler(self,self.onShowJumpVipDialog))    --跳转vip弹框
   ...
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_ShareTelegram,handler(self,self.onShowTelegram))     --分享到飞机
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_TURNTABLEHISTORY,handler(self,self.onShowTurnTableHistory))    --打开幸运转盘历史记录
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_VipHelperMoneyConfig,handler(self,self.onShowVipHelperMoneyConfig))    --打开救援金配置界面
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_VipHelpMoney,handler(self,self.onShowVipHelpMoney))    --打开救援金界面
end

function SubLayerJump:onRemoveListen()
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_VipHelpMoney)  
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_VipHelperMoneyConfig)  
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_TURNTABLEHISTORY)  
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_ShareTelegram)  
   ...
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_GIFT_CODE_SHOP)   --激活码限时礼包商城界面
    ...
    G_event:RemoveNotifyEvent(G_eventDef.UI_JUMP_VIP_DIALOG)
end
...
function SubLayerJump:onShowJumpVipDialog(args)
   local parent = cc.Director:getInstance():getRunningScene()
   if args.name then
      local pLayer = parent:getChildByName(args.name)
      if pLayer then
   ...
   appdf.req(luaPath.JumpVipDialog).new(args)  
end
...
--分享到飞机
function SubLayerJump:onShowTelegram(args)
   appdf.req(luaPath.ShareTelegramLayer).new(args)
end

--打开幸运转盘历史记录
function SubLayerJump:onShowTurnTableHistory(args)
   appdf.req(luaPath.TurnTableHistory).new(args)
end

--打开vip救援金配置界面
function SubLayerJump:onShowVipHelperMoneyConfig(args)
   appdf.req(luaPath.VipHelperMoneyConfig).new(args)
end

--打开vip救援金界面
function SubLayerJump:onShowVipHelpMoney(args)
   appdf.req(luaPath.VipHelpMoney).new(args)
end
