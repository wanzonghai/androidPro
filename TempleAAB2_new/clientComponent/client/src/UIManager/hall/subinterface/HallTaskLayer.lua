...
function HallTaskLayer:onFinishTaskClick(data)
    ...
    G_ServerMgr:C2S_RequestTaskReward(data.iTaskID)
end
...
function HallTaskLayer:getGameTypeData(gameId)
    ...
    local gameType = nil
    local gameIconConfig = HallTableViewUIConfig.All
...
function HallTaskLayer:OnUpdateDownProgress(args)
    if self.updateInfo and self.updateInfo.gameKindID and self.updateInfo.gameKindID == args.gameId and self.updateInfo.percentNode and not tolua.isnull(self.updateInfo.percentNode) then
...
function HallTaskLayer:OnUpdateDownSuccess(args)
    if self.updateInfo and self.updateInfo.percentNode and not tolua.isnull(self.updateInfo.percentNode) then
        self.updateInfo.percentNode:hide()
    end
end
...
function HallTaskLayer:onActivenessRewardClick(target)
    ...
    G_ServerMgr:C2S_RequestActivenessReward(dwConfigID)
end
