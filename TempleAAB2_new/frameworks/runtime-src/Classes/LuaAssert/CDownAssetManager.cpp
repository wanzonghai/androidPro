...
//构造函数
CDownAssetManager::CDownAssetManager(const char* szUrl,const char* szFileName,const char* szSavePath,int nHandler,int nTimeout)
: m_szDownUrl (szUrl)
...
//创建函数
void CDownAssetManager::DownFile(const char* szUrl,const char* szFileName,const char* szSavePath,int nHandler,int nTimeout)
{
	CDownAssetManager *pDownAsset = new CDownAssetManager(szUrl,szFileName,szSavePath,nHandler,nTimeout);
	//自动释放
...
//通知UI
void CDownAssetManager::Notify(int wMain,int wSub, std::string fileName)
{
	if (m_nHandler != 0)
	{
		if (!fileName.empty()) {
			Global_PushSyncData(MSG_HTTP_DOWN, m_nHandler, (WORD)wMain, (WORD)wSub, fileName.c_str(), fileName.length()+1);
		}
		else {
...
void CDownAssetManager::DownRun()
{
	...
	do
	{
		...
		m_curlHandle = _curl;
		// curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYPEER, 0L); // SKIP_PEER_VERIFICATION
        // curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYHOST, 0L); // SKIP_HOSTNAME_VERIFICATION
        // curl_easy_setopt(_curl, CURLOPT_CONNECTTIMEOUT, obj->conn_tmo);
        // curl_easy_setopt(_curl, CURLOPT_TIMEOUT, m_nTimeout);
		...
		curl_easy_setopt(_curl, CURLOPT_LOW_SPEED_TIME, 30);	//控制多少秒传送CURLOPT_LOW_SPEED_LIMIT
		curl_easy_setopt(_curl, CURLOPT_FOLLOWLOCATION, 1 );				//设置支持302重定向
		curl_easy_setopt(_curl, CURLOPT_CONNECTTIMEOUT, m_nTimeout);					//设置超时
		...
		//通知结果
		Notify((res != CURLE_OK)?DOWN_ERROR_NET:DOWN_COMPELETED,res, m_szFileName);
	}while(false);
