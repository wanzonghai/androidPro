...
protected:
	//构造函数
	DLL_LOCAL CDownAssetManager(const char* szUrl,const char* szFileName,const char* szSavePath,int nHandler,int nTimeout);
...
public:
	//创建函数
	DLL_LOCAL static void DownFile(const char* szUrl,const char* szFileName,const char* szSavePath,int nHandler,int nTimeout);
...
protected:
	//通知UI
	DLL_LOCAL void Notify(int wMain,int wSub, std::string fileName = "");
	//下载函数
