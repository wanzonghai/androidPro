...
--OneGame
g_OneGame = appdf.req(appdf.CLIENT_SRC.."ExtraGame.OneGame")
...
function LogonScene:onEnterTransitionFinish()    
    --上报AD应用打开事件
    g_MultiPlatform:getInstance():threeLogEvent("ad_app_open")
    ...
    self:CheckInitNetwork()

    -- -- --网络监测完成，缓出登录部分
    -- local pComplete = function()
    --     showNetLoading(nil,nil,128)
    --     self.PanelTips:hide()
...
--检测IP获取完成,并且初始化Network完成
function LogonScene:CheckInitNetwork()
    self.schedulerIP = scheduler:scheduleScriptFunc( function()
        print("InitNetworkOver = ",InitNetworkOver)
        if MyIP and MyIP~="127.0.0.1" and InitNetworkOver then
            if self.schedulerIP then
                scheduler:unscheduleScriptEntry(self.schedulerIP)
                self.schedulerIP = nil
            end
            dismissNetLoading()
            GlobalData.MyIP = MyIP
            self:CheckNetwokCount()
        end
    end , 0.1, false)
end

--检测网络通道可用个数
function LogonScene:CheckNetwokCount()
    self.remainTime = 8
    self.schedulerID = scheduler:scheduleScriptFunc(handler(self,function(dt)
        self.remainTime = self.remainTime - 0.1
        local pCount = CCGetReadyServerCount and CCGetReadyServerCount() or 1
        print("pCount = ",pCount)
        if self.remainTime < 0 or pCount>0 then
            self.remainTime = 0
        end
...
function LogonScene:DealAutoLogin()
    ...
    elseif GlobalUserItem.LoginType == 2 or GlobalUserItem.LoginType == 3 then
        --社交登录
        if GlobalUserItem.LoginData then
            self:onThirdAuthCallback()
        end
...
function LogonScene:onPhoneLogin(pData)
    ...
    self.LoginData = {1}
    --手机登录
    g_MultiPlatform:getInstance():threeLogEvent("ad_click_phone_login") 
...
function LogonScene:onGoogleLogin()
    --Google登录
    g_MultiPlatform:getInstance():threeLogEvent("ad_click_google_login") 
...
function LogonScene:onGuestLogin()
    ...
    self.LoginData = {0}
    --游客登录
    g_MultiPlatform:getInstance():threeLogEvent("ad_click_guest_login")    
    G_ServerMgr:checkConnectStatus()
...
function LogonScene:onThirdAuthCallback(args)
    self:ShowloadingBar()
    local data = args and args.data or GlobalUserItem.LoginData
...
function LogonScene:LogonSuccess(args)    
    ...
    if self.LoginData[1] == 0 then   
        --游客登录
        g_MultiPlatform:getInstance():threeLogEvent("ad_guest_login_success")             
    elseif self.LoginData[1] == 1 then
        --手机登录
        g_MultiPlatform:getInstance():threeLogEvent("ad_phone_login_success")    
    elseif self.LoginData[1] == 2 then
        --FB登录          
    elseif self.LoginData[1] == 3 then  
        --Google登录
        g_MultiPlatform:getInstance():threeLogEvent("ad_google_login_success")    
    end 
    ...
    if GlobalUserItem.isNewUser then
        --上报AD注册事件
        g_MultiPlatform:getInstance():threeLogEvent("ad_register")
    end
end
