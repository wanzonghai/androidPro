...
function ShareTurnTableLayer:init()
  ...
  --  self:showAnimation()
    ...
    self._playerHistoryData = {}

    self:updateLeftGift()
    G_event:NotifyEvent(G_eventDef.UI_SHARESTEP,self)
...
function ShareTurnTableLayer:onEnter()
    ShareTurnTableLayer.super.onEnter(self)
    ...
    if eventType == ccui.TouchEventType.ended then
        ...
        elseif name == "howBtn" then
            if self._lastLeftIndex == 2 then
                G_event:NotifyEvent(G_eventDef.UI_SHARESTEP,self)
                G_event:NotifyEvent(G_eventDef.RECEIVE_VIP_ANDGIFT2) 
...
function  ShareTurnTableLayer:luckList(pData)
    ...
    for k = 1,#lsItems do
        ...
        self._playerHistoryData[#self._playerHistoryData + 1] = data
...
function ShareTurnTableLayer:initNode1ListView(result)
    ...
    local width = self.cloneGiftNode1:getContentSize().width
    local sumWidth = 10
    local curWidth = 14
    ...
    for k, val in ipairs(result.lsStatus) do
        ...
        giftNode:setPosition(cc.p(sumWidth,150))
        sumWidth = sumWidth + width + curWidth
        index = index + 1
        if k == 4 then
...
function ShareTurnTableLayer:setGiftBoxItemInfo(item,value,index,scoreConfig)
    ...
    local spineNode = item:getChildByName("spineNode")
    ...
    textBody:setPositionY(70)
    cloneGiftPriceText:setTextColor(cc.c3b(192,35,64))
  ...
  --  cloneLoadBar:loadTexture("client/res/ShareTurnTable/line01.png",1)
    ...
    if value == 0 then    
        printf("不可领取")
        cloneGiftPriceText:setTextColor(cc.c3b(78,78,78))
        if spineNode._spine then
            spineNode._spine:hide()
            cloneIcon:show()
        end
    ...
    elseif value == 1 then
        printf("可领取") -- 1
        if not spineNode._spine then
            local spine = self:addSpine(spineNode,"lihe")
            spine:setAnimation(0,"daiji"..index,true)
            spineNode._spine = spine
            cloneIcon:hide()
        else
            spineNode._spine:show() 
            cloneIcon:hide() 
        end
        -- cloneLoadBar:loadTexture("client/res/ShareTurnTable/line01.png",1)
...
function ShareTurnTableLayer:setRichText()
    ...
    richText:setContentSize(cc.size(880,400))
...
function ShareTurnTableLayer:setRichText2()
    ...
    richText:ignoreContentAdaptWithSize(true)
    richText:setAnchorPoint(cc.p(0,0.5))
    local re1 = ccui.RichElementText:create(1,cc.c3b(255,255,255),255,"2.  Os jogadores C e D convidados pelos seus amigos.  se eles fizerem uma recarga,","Helvetica",30) 
