...
function SetGameIcon:setParent(parent,scene,hallPlist)
    tableViewParent = parent
    LobbyHallPlist = hallPlist
    self._scene = scene
    self._gameIconSizeConfig = {}
    self._tableView = nil
...
function SetGameIcon:initGameItem(index,item)  
    ...
    if gameConfig then              
        ...
        local ID = oneLine.ID
        local iconNode = self._iconNameNodeConfig[Name] or self:createNewNode(oneLine,index,Name)
        ...
        local parent = iconNode:getParent()
        if parent then 
            iconNode:removeFromParent() 
        end
        item:addChild(iconNode)
        local _Button_1 = iconNode._Button_1
        if _Button_1 then   
            local pngName = "client/res/Lobby/GameIcon/"..Name..".png"
            local frame = spriteFrameCache:getSpriteFrame(pngName)         
            if frame then
                _Button_1:loadTextureNormal(pngName,1)
                _Button_1:loadTexturePressed(pngName,1)
            else
...
function SetGameIcon:createNewNode(data,index,Name)
    ...
    local function onTouchGame(sender,eventType)
        ...
        if eventType == ccui.TouchEventType.began then
            ...
            local gameId = pData and pData.ID
            ...
            local vipImg = self._vipLimitBtn[gameId]
            if vipImg then
                vipImg:stopAllActions()
                vipImg:runAction(cc.ScaleTo:create(0.12,0.96))
            end
        ...
        elseif eventType == ccui.TouchEventType.ended then
            ...
            if vipImg then
                vipImg:stopAllActions()
                vipImg:runAction(cc.ScaleTo:create(0.12,1))
            end
            parent:stopAllActions()
...
function SetGameIcon:createVipLimitImg(iconNode,gameId,Type)
    ...
    if Type == HallTableViewUIConfig.GameType.EG then                 --如果是EG游戏
       ...
       -- local vipImg = ccui.ImageView:create(string.format("client/res/VIP/GUI/%d.png",1),ccui.TextureResType.plistType)
        local vipImg = cc.CSLoader:getInstance():createNodeWithFlatBuffersFile("Lobby/maskNode.csb")
        local maskBack = vipImg:getChildByName("maskBack")
        maskBack:setOpacity(130)
        local vipText = vipImg:getChildByName("vipText")
        vipText:setString(tostring(level))
        vipImg:setPosition(0,0)
...
function SetGameIcon:hideDownImgByID(pID)
    local downImg = self._updateBtn[pID]
    if downImg then
        downImg:hide()
    end
end
...
function SetGameIcon:createUpdateNode(iconNode,gameId,index)
    ...
    if GlobalData.DownloadGameList[gameId] then
        pNodeUpdate:show()
        pNodeUpdate:setUpdatePercent(0)
        pNodeUpdate:dispatchEndCallBack(true)
    else
        pNodeUpdate:hide()
    end
...
function SetGameIcon:hall2RoomList(iconNode)
    ...
    if isUpdate then
        ...
        G_event:NotifyEvent(G_eventDef.UI_GAME_UPDATE,args)
    else  
        self:hideDownImg(iconNode)
        ...
        if gameType == HallTableViewUIConfig.GameType.SelectRoom then          --如果还需要选场
            ...
            if pType == HallTableViewUIConfig.GameType.EG then                 --如果是EG游戏                
                ...
                -- if pFlag then                    
                    if GlobalUserItem.VIPLevel and GlobalUserItem.VIPLevel >= ylAll.SERVER_UPDATE_DATA.easy_game_threshold then                    
                        ...
                        if not pFlag then
                            local ext_json = {gameId = gameId,roomId = gameId*1000+90+1}
                            EventPost:addCommond(EventPost.eventType.PV,"点击进入API游戏",nil,nil,ext_json)
                            G_event:NotifyEvent(G_eventDef.UI_START_GAME,{roomMark = gameId*1000+90+1,quickStart = false})
                        end
                    else                
            ...
            elseif pType == HallTableViewUIConfig.GameType.PG then
                dump(gameData)
                G_event:NotifyEvent(G_eventDef.UI_START_GAME,{roomMark = gameId*1000+100+1,quickStart = false})
            elseif pType == HallTableViewUIConfig.GameType.ONE then
                dump(gameData)
                G_event:NotifyEvent(G_eventDef.UI_START_GAME,{roomMark = gameId*1000+110+1,quickStart = false})
...
function SetGameIcon:setIconStatus(node,gameId,Type)
    ...
    if      Type ~= HallTableViewUIConfig.GameType.EG 
        and Type ~= HallTableViewUIConfig.GameType.ONE 
...
function SetGameIcon:setIconType(index)
    ...
    if index == 1 then                  --热,要有活动界面
        self._iconConfig = clone(HallTableViewUIConfig.Hot)
    elseif index == 2 then              --slots
        self._iconConfig = clone(HallTableViewUIConfig.Slot)
    elseif index == 3 then              --Lazer
        self._iconConfig = clone(HallTableViewUIConfig.Lazer)
    end
...
function SetGameIcon:addSpriteFrames()
    if not LobbyHallPlist then return end
    for k,v in pairs(LobbyHallPlist) do
        local plistName = string.format("client/res/%s.plist",v) 
        local plistPng = string.format("client/res/%s.png",v) 
