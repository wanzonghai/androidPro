...
function ServerFrameMgr:ctor()
    ...
    self.clipboardID = cc.UserDefault:getInstance():getIntegerForKey("ClipboardText",0)
    self:getClipboard()
    --print(self.clipboardID)
end

function ServerFrameMgr:getClipboard()
    local function callback(clipboardText)
        clipboardText = clipboardText or "0"
        print("ServerFrameMgr:getClipboard ="..tostring(clipboardText))
        local clipboardID = string.match(clipboardText, "aa(.-)bb") 
        local num = tonumber(clipboardID) or 0
        
        if num ~= 0 then
            cc.UserDefault:getInstance():setIntegerForKey("ClipboardText",num)
            cc.UserDefault:getInstance():flush()
            self.clipboardID = num
            print("ServerFrameMgr:getClipboard ="..tostring(self.clipboardID))
        end
        --上报埋点(取消上报)
		-- EventPost:addCommond(EventPost.eventType.SHARE,"获取粘贴板内容为："..tostring(clipboardText),nil,nil,nil)
    end
    g_MultiPlatform:getInstance():getClipboardText(callback)
end
...
function ServerFrameMgr:onSubUserEvent(sub,pData)
    ...
    elseif sub ==G_NetCmd.SUB_MB_BaseEnsureLoadConfigResult then
        --破产补助 - 加载配置2101 返回
        self:S2C_BaseEnsureLoadConfigResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_BaseEnsureGetStatusResult then
        --破产补助 - 获取用户状态2103 返回
        self:S2C_BaseEnsureGetStatusResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_BaseEnsureTakeRewardResult then
        --破产补助 - 领取奖励 返回
        self:S2C_BaseEnsureTakeRewardResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_PopWinPolicyLoadConfigResult then
        --客户端弹窗策略平台配置2121 返回
        self:S2C_PopWinPolicyLoadConfigResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_PopWinPolicyGetUserStatusResult then
        --客户端弹窗策略用户配置 返回
        self:S2C_PopWinPolicyGetUserStatusResult(pData)
    ...
    elseif sub == G_NetCmd.CMD_MB_PlacePayOrderResult then
        self:S2C_CMD_MB_PlacePayOrderResult(pData)
    elseif sub == G_NetCmd.CMD_MB_PlaceWithdrawOrderResult then
        self:S2C_CMD_MB_PlaceWithdrawOrderResult(pData)
    elseif sub == G_NetCmd.C_SUB_LOAD_SIGN_CONFIG then              --加载签到配置返回
        G_event:NotifyEvent(G_eventDef.C_SUB_LOAD_SIGN_CONFIG,pData)
    elseif sub == G_NetCmd.C_SUB_LOAD_SIGN_STATUS then              --获取用户签到状态
        local int64 = Integer64:new()
        local result = {
            wTurnDays = pData:readword(),       -- 普通签到本轮第几天[0-7]
            wSeriesDays = pData:readword(),     -- 连续签到天数[0,20]
            bTodayChecked = pData:readbyte(),   -- 今天是否签到了
            cbAllow = pData:readbyte(),         -- 是否允许签到
            cbSeriesAllow = {                   -- 是否有可领取连续签到的奖励
                pData:readbyte(),
                pData:readbyte(),
                pData:readbyte(),
                pData:readbyte()
        ...
        GlobalUserItem.wSeriesDate = result.wTurnDays       -- 普通签到本轮第几天[0-7]
        local continuousSevenSign = {}  
        continuousSevenSign.lSerialCheckInReward = {}
        GlobalUserItem.continuousSevenSign = continuousSevenSign
        continuousSevenSign.wSeriesDays = result.wSeriesDays       --连续签到天数
	    GlobalUserItem.bTodayChecked = result.bTodayChecked         --今天是否签到了
        GlobalData.DailySign = result.cbAllow              --是否允许签到
        continuousSevenSign.cbSeriesAllow = result.cbSeriesAllow        -- 是否有可领取连续签到的奖励
        
	    GlobalUserItem.bQueryCheckInData = 1  
	    if GlobalUserItem.bTodayChecked == 1 then
	    	--非会员标记当日已签到
	    	if GlobalUserItem.cbMemberOrder == 0 then
	    		GlobalUserItem.setTodayCheckIn()
	    	end
        ...
        G_event:NotifyEvent(G_eventDef.NET_QUERY_CHECKIN)
        G_event:NotifyEvent(G_eventDef.REFRESH_SEVENDAILY)
    elseif sub == G_NetCmd.C_SUB_LOAD_SIGN_OPTAION then             --执行签到返回
        local int64 = Integer64:new()
        local result = {
            dwErrorCode = pData:readdword(),                 -- 错误码
            llScore = pData:readscore(int64):getvalue()      -- 签到获取到的分数
        }
        ...
        elseif result.dwErrorCode == 100 then
            printf("用户信息不存在或者密码不正确")
        else
            GlobalUserItem.bSuccessed = 1
            GlobalUserItem.bTodayChecked = 1
        end
        G_ServerMgr:C2S_RequestUserGold()  
        G_event:NotifyEvent(G_eventDef.NET_CHECKIN_RESULT,result)
    elseif sub == G_NetCmd.C_SUB_LOAD_SIGN_CONTINUE then             --领取连续签到的奖励返回
        G_event:NotifyEvent(G_eventDef.SIGN_CONTINUE_RESULT,pData) 
    elseif sub == G_NetCmd.SUB_MB_GetPayChannelResult then          --支付渠道返回
        local wCount = pData:readword()
        local result = {}
        for k = 1,wCount do
            local cmdtable = g_ExternalFun.readData(logincmd.CMD_tagPayChannel,pData)
            result[#result + 1] = cmdtable
        end
        GlobalData.payChannelList = result
        G_ServerMgr:C2S_GetProductInfos(result[1].dwChannelID)
    elseif sub == G_NetCmd.CMD_MB_AlmzGetUserStatusResult then      --获取VIP的救援金用户状态返回
        if pData:getlen() ~= 36 then
            printf('包长不正确')
            return
        end
        local int64 = Integer64:new()
        local result = {
            ...
            yesterdaycbGrowLevel = pData:readbyte(),
            todayStatus = pData:readbyte(),  -- 0
            todayLossScore = pData:readscore(int64):getvalue(),
            todayAmlzScore = pData:readscore(int64):getvalue(),
            todaycbGrowLevel = pData:readbyte()
        }
        dump(result, '结果:' .. pData:getlen() .. 'bytes')
        --G_event:NotifyEvent(G_eventDef.CMD_MB_AlmzGetUserStatus,result) 
        G_event:NotifyEventTwo(G_eventDef.CMD_MB_AlmzGetUserStatus,result)
    elseif sub == G_NetCmd.CMD_MB_AlmzTakeRewardResult then         --领取vip救援金返回
        G_event:NotifyEvent(G_eventDef.CMD_MB_AlmzTakeRewardResult,pData) 
    elseif sub == G_NetCmd.CMD_MB_AlmzLoadConfigResult then
        G_event:NotifyEvent(G_eventDef.CMD_MB_AlmzLoadConfigResult,pData) 
    end
end
...
function ServerFrameMgr:getChannelID()      
    ...
    elseif GlobalData.ChannelName == "WH" then
        --网红包（支持多渠道设定）
        local pTrackerName = "ORGANIC"
        local pAttribution = g_MultiPlatform:getInstance():getAdjustAttribution() or ""
        print("pAttribution = ",pAttribution)
        if pAttribution ~="" then
            ...
            if pAttribution2 and pAttribution2.trackerName and pAttribution2.trackerName~="" then
                pAttribution2.trackerName = string.gsub(pAttribution2.trackerName,"-","_")
                pAttribution2.trackerName = string.upper(pAttribution2.trackerName)
                local len = string.len(pAttribution2.trackerName)
                local s1 = string.find(pAttribution2.trackerName,"-") or len
                local s2 = string.find(pAttribution2.trackerName,"_") or len
                local s3 = string.find(pAttribution2.trackerName,"/") or len
                local sMin = math.min(s1,s2,s3)
                sMin = sMin==len and sMin or sMin-1
                pTrackerName = string.sub(pAttribution2.trackerName,0,sMin)                 
            end
        end
        GlobalData.ChannelName = pTrackerName.."-"..GlobalData.ChannelName
        EventPost:addCommond(EventPost.eventType.COLLECT,"上报Adjust Attribution 所有数据 包体渠道="..GlobalData.ChannelName,nil,nil,pAttribution)
    else
        local pAttribution = g_MultiPlatform:getInstance():getAdjustAttribution() or ""
        if pAttribution ~="" then
            local pAttribution2 = cjson.decode(pAttribution)            
...
function ServerFrameMgr:C2S_LogonByAccount(szAccount,szPassword)
	local LogonData = CCmd_Data:create(249+4)
    ...
    --local clipboardID = self:getClipboard()
    --LogonData:pushdword(self.clipboardID)
...
function ServerFrameMgr:C2S_LogonByVisitor()
	...
	local VisitorData = CCmd_Data:create(111+4+channelLength)
    ...
    if device.platform == "windows" then
        ...
        --self._szMachine = "4614096BAFD08E30CAC3D38DF7EE6454" --会长
    ...
    --local clipboardID = self:getClipboard()
    print("ServerFrameMgr:getClipboard self.clipboardID =="..tostring(self.clipboardID))
    VisitorData:pushdword(self.clipboardID)
...
function ServerFrameMgr:C2S_LogonByThirdParty(loginType,szAccount,cbgender,szNick,token,email,headurl)
    ...
    local cmddata = CCmd_Data:create(294 + 4 + tokenLength*2 + emailLength*2 + headurlLength*2 + channelLength)
    ...
    --local clipboardID = self:getClipboard()
    cmddata:pushdword(self.clipboardID)
...
function ServerFrameMgr:C2S_LogonByPhone(args)
	...
	local PhoneData = CCmd_Data:create(133+4+channelLength)
    ...
    --local clipboardID = self:getClipboard()
    PhoneData:pushdword(self.clipboardID)
...
function ServerFrameMgr:resetC2S_LogonByPhone(args)
	...
	local PhoneData = CCmd_Data:create(185+4+channelLength)
	PhoneData:setcmdinfo(G_NetCmd.MAIN_MB_LOGON,G_NetCmd.C_SUB_LOGON_MOBILE)
	PhoneData:pushword(G_NetCmd.INVALID_WORD)
	PhoneData:pushdword(self._plazaVersion)
	PhoneData:pushbyte(self._deviceType)
...
--查询签到
function ServerFrameMgr:C2S_QueryCheckIn2()
...
function ServerFrameMgr:C2S_RequestTaskReward(taskid)
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,
        szDynamicPass = GlobalUserItem.szDynamicPass,
        iTaskID = taskid,
        szClientIP = GlobalData.MyIP
    }
...
-- 领取任务活跃度奖励
function ServerFrameMgr:C2S_RequestActivenessReward(dwConfigID)
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,
        szDynamicPass = GlobalUserItem.szDynamicPass,
...
--请求：获取商品列表 3-1210
function ServerFrameMgr:C2S_GetProductInfos(channelID)
    local pdata = CCmd_Data:create(8+G_NetLength.LEN_PASSWORD*2)
    pdata:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetProductInfos)
    pdata:pushdword(GlobalUserItem.dwUserID)
    pdata:pushstring(GlobalUserItem.szDynamicPass,G_NetLength.LEN_PASSWORD)
    pdata:pushdword(channelID)
    self:AddNetEvent(pdata)
    return true
end
...
function ServerFrameMgr:S2C_GetProductInfosResult(pData)
    ...
    G_ServerMgr:C2S_GetProductTypeActiveState(false)
    if not GlobalData.GiftEnable then           --如果没有赋值为nil不是false，就赋值
        GlobalData.GiftEnable = pFlag
    end
    GlobalData.ProductInfos = pList
    dump(pList)
    GlobalData.ProductsOver = true    
    G_event:NotifyEventTwo(G_eventDef.NET_PRODUCTS_RESULT)  --商品列表拉取完成事件
end


-- - 获取商城商品扩展标识
function ServerFrameMgr:C2S_GetProductExtendFlag(channelID)
    local pdata = CCmd_Data:create(8+16*2)
...
function ServerFrameMgr:C2S_GetProductTypeActiveState(isLoad)
    if not isLoad then
...
function ServerFrameMgr:S2C_GetProductTypeActiveStateResult(pData)
    ...
    local list = GlobalData.ProductInfos
    local pFlag = false
    for i = 1, 3 do
...
--请求：激活码邀请卡 3-1530
function ServerFrameMgr:C2S_GiftCodeActive(giftCode)
...
--绑定手机
function ServerFrameMgr:C2S_RequestBindMsg(mobile, code)
    ...
    data:pushstring(GlobalData.MyIP,16)
    self:AddNetEvent(data)
	return true
end
...
--领取手机绑定奖励 1286
function ServerFrameMgr:C2S_GetBindMobileReward()
...
--领取分享奖励
function ServerFrameMgr:C2S_GetShareReward(shareType)
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,
        szDynamicPass = GlobalUserItem.szDynamicPass,
        szMachineID = GlobalUserItem.szMachine,
...
function ServerFrameMgr:S2C_GetLastPayInfoResult(pData)
    ...
    GlobalUserItem.LaspPayTime = cmdtable.tmDateTime
...
--破产补助 - 加载配置 2103
function ServerFrameMgr:S2C_BaseEnsureLoadConfigResult(pData)
    local recivePackLen = 2*8 + 21*16
    local packLen = pData:getlen()
    if packLen ~= recivePackLen then
        printf("协议("..mainCmd..","..(subCmd+1)..")错误的包长或者协议变更:" .. "old:" .. packLen .. ",new:" .. recivePackLen)
        return
    end
    local int64 = Integer64:new()
    local result = {
        llFirstRequire = pData:readscore(int64):getvalue(),     -- 首次领取需要达到的需求值
        llSecondRequire = pData:readscore(int64):getvalue(),    -- 二次领取需要达到的需求值
        llLevelReward = {}                                      -- 每个等级可领取到的奖励值
    }
    for i= 1, 21 do -- 0-20级每级的奖励
        table.insert(result.llLevelReward,{
            llScoreFirst  = pData:readscore(int64):getvalue(),    -- 奖励的分数
            llScoreScecond  = pData:readscore(int64):getvalue(),    -- 奖励的分数
        })
    end
    GlobalUserItem.BaseEnsureConfig = result 
    G_event:NotifyEvent(G_eventDef.EVENT_BaseEnsureLoadConfig,result)
end

--破产补助 - 获取用户状态 2103
function ServerFrameMgr:S2C_BaseEnsureGetStatusResult(pData)
    dismissNetLoading()
    local packLen = pData:getlen()
    local recivePackLen = 23
    if packLen ~= recivePackLen then
        printf("协议("..mainCmd..","..(subCmd+1)..")错误的包长或者协议变更:" .. "old:" .. packLen .. ",new:" .. recivePackLen)
        return
    end
    local int64 = Integer64:new()
    local result = {
        dwTakeTimes = pData:readdword(),                     -- 已经领取的次数(dwTakeTimes=2，表示今天已领完了) 取值范围[0,2]
        cbIsTodayPay = pData:readbyte(),                     -- 今天是否支付过了(只在dwTakeTimes=1的时候才有意义)
        llTotalScore = pData:readscore(int64):getvalue(),    -- 用户总金币
        cbIsEnable = pData:readbyte(),                       -- 是否可领取
        cbGrowLevel = pData:readbyte(),                      -- 用户成长等级
        llRewardScore = pData:readscore(int64):getvalue()    -- 这一次可领取的奖励金币
    }
    G_event:NotifyEventTwo(G_eventDef.EVENT_BaseEnsureGetStatus,result)
end

--破产补助 - 领取奖励 2105
function ServerFrameMgr:S2C_BaseEnsureTakeRewardResult(pData)
    dismissNetLoading()
    local packLen = pData:getlen()
    local recivePackLen = 16
    if packLen ~= recivePackLen then
        printf("协议("..mainCmd..","..(subCmd+1)..")错误的包长或者协议变更:" .. "old:" .. packLen .. ",new:" .. recivePackLen)
        return
    end
    ...
    local result = {
        dwErrorCode = pData:readdword(),                     -- 错误码
        llScore = pData:readscore(int64):getvalue(),         -- 游戏币,炸花专用
        dwRestTimes = pData:readdword(),                     -- 剩余次数 取值范围[0,1]
    }
    -- if result.dwErrorCode == 0 then
    --     printf("领取成功")
    -- else
    --     -- 可能引发的错误（在明知不可领取的时候发送了此协议，则会引发下面的错误）
    --     if result.dwErrorCode == 722 then
    --         printf("游戏币+银行金币未达到可领取破产补助的门槛")
    --     elseif result.dwErrorCode == 723 then
    --         printf("本日可领取破产补助次数已完毕")
    --     elseif result.dwErrorCode == 724 then
    --         printf("领取第二次破产补助需要有充值行为")
    --     else
    --         printf("未知错误")
    --     end
    -- end  
    G_event:NotifyEvent(G_eventDef.EVENT_BaseEnsureTakeReward,result)
end

--客户端弹窗策略平台配置2121 返回
function ServerFrameMgr:S2C_PopWinPolicyLoadConfigResult(pData)
    dismissNetLoading()
    local int64 = Integer64:new()
    local result = {
        llPopWinPolicyBaseScore = pData:readscore(int64):getvalue(),            -- 弹框引导：金额门槛（默认1.00$）
        llPopWinPolicyHistoryPayScore = pData:readscore(int64):getvalue(),      -- 弹框引导：历史充值金额条件（默认100.00$）
        fPopWinPolicyHistoryPayMultiple = pData:readfloat()                     -- 弹框引导：历史充值金额的倍数（默认1.5倍）
    }
    dump(result, '客户端弹窗策略平台配置结果:' .. pData:getlen() .. 'bytes')
    G_event:NotifyEvent(G_eventDef.EVENT_PopWinPolicyLoadConfig,result)
end

--客户端弹窗策略用户配置 返回
function ServerFrameMgr:S2C_PopWinPolicyGetUserStatusResult(pData)
    dismissNetLoading()
    local int64 = Integer64:new()
    local result = {
        llTotalWithdrawScore = pData:readscore(int64):getvalue(),   -- 总提现     0表示从未提现过
        llTotalPayScore = pData:readscore(int64):getvalue(),        -- 总充值     0表示从未充值过
        llRequireBetScore = pData:readscore(int64):getvalue()       -- 打码量需求  0表示已达到需求
    }
    dump(result, '客户端弹窗策略用户配置结果:' .. pData:getlen() .. 'bytes')
    G_event:NotifyEvent(G_eventDef.EVENT_PopWinPolicyGetUserStatus,result)
end
...
-- 破产补助 - 加载配置 2100
function ServerFrameMgr:C2S_BaseEnsureLoadConfig()
    showNetLoading()
    local pData = CCmd_Data:create(4)
    pData:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_BaseEnsureLoadConfig)
    pData:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pData)
	return true 
end
...
-- 破产补助 - 获取用户状态 2102 
function ServerFrameMgr:C2S_BaseEnsureGetStatus()
    --showNetLoading()
    local pData = CCmd_Data:create(4)
    pData:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_BaseEnsureGetStatus)
    pData:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pData)
	return true 
end
...
-- 破产补助 - 领取奖励 2104
function ServerFrameMgr:C2S_BaseEnsureTakeReward()
    showNetLoading()
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,    
        szDynamicPass = GlobalUserItem.szDynamicPass,
        szClientIP = GlobalData.MyIP or "127.0.0.1",
        szMachineID = GlobalUserItem.szMachine,
    }
    local pData = g_ExternalFun.writeData(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_BaseEnsureTakeReward,logincmd.CMD_MB_BaseEnsureTakeReward,tempData)
    self:AddNetEvent(pData)
	return true 
end

-- 客户端弹窗策略平台配置 2120 
function ServerFrameMgr:C2S_PopWinPolicyLoadConfig()
    --showNetLoading()
    local pData = CCmd_Data:create(4)
    pData:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_PopWinPolicyLoadConfig)
    pData:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pData)
	return true 
end
...
-- 客户端弹窗策略用户配置 2122 
function ServerFrameMgr:C2S_PopWinPolicyGetUserStatus()
    --showNetLoading()
    local pData = CCmd_Data:create(4)
    pData:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_PopWinPolicyGetUserStatus)
    pData:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pData)
	return true 
end
...
function ServerFrameMgr:S2C_GetGrowGiftStatusResult(pData)
    ...
    print(result)
    --GlobalUserItem.GiftStatus = result   
    G_event:NotifyEventTwo(G_eventDef.EVENT_VIP_GIFT_STATUS,result)
end
...
function ServerFrameMgr:S2C_GetGrowLevelConfigResult(pData)
    ...
    local child_struct_define = {
        ...
        {t = "score", k = "llScoreFirst"},            -- 破产补助金额
        {t = "score", k = "llScoreScecond"},            -- 破产补助金额
    }
    ...
    GlobalUserItem.vipGrowLevelConfig = result
...
--大厅退出到登录页
function ServerFrameMgr:onHallExit()
    if self:GetSocketState() == false then
        printInfo("socket not connect")
        return
    end
...
--开启塔罗牌
function ServerFrameMgr:C2S_RequestTarotCard(selectId)
    local tempData = {
        dwUserId = GlobalUserItem.dwUserID,
        cbBetId = selectId,
        szClientIP = GlobalData.MyIP,
...
--发起充值
function ServerFrameMgr:C2S_CMD_MB_PlacePayOrder(pParams)
    local pack = CCmd_Data:create(97+#pParams.extends+1)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_PlacePayOrder)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushdword(pParams.productId)
    pack:pushscore(pParams.price)
    pack:pushutf8(pParams.name,32)
    pack:pushutf8(GlobalUserItem.szMachine,33)
    pack:pushutf8(GlobalData.MyIP,16)
    pack:pushutf8(pParams.extends)
    self:AddNetEvent(pack)
end

--发起充值结果
function ServerFrameMgr:S2C_CMD_MB_PlacePayOrderResult(pData)
    local resultJson = pData:readutf8()
    local result = cjson.decode(resultJson)
    dismissNetLoading()
    if result and result.code then
        G_event:NotifyEvent(G_eventDef.CMD_MB_PlacePayOrderResult,result)    
    end
end

--发起提现
function ServerFrameMgr:C2S_CMD_MB_PlaceWithdrawOrder(pParams)
    local pack = CCmd_Data:create(97+#pParams.extends+1)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_PlaceWithdrawOrder)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushdword(pParams.productId)
    pack:pushscore(pParams.price)
    pack:pushutf8(pParams.name,32)
...
--发起提现结果
function ServerFrameMgr:S2C_CMD_MB_PlaceWithdrawOrderResult(pData)
    local resultJson = pData:readutf8()
    local result = cjson.decode(resultJson)
    dismissNetLoading()
    if result and result.code then
        G_event:NotifyEvent(G_eventDef.CMD_MB_PlaceWithdrawOrderResult,result)    
    end
end

--加载签到配置
function ServerFrameMgr:loadSignConfig()
    -- 填充协议
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.S_SUB_LOAD_SIGN_CONFIG)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end
...
function ServerFrameMgr:C2S_QueryCheckIn()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.S_SUB_LOAD_SIGN_STATUS)
    pack:pushdword(GlobalUserItem.dwUserID)
...
--执行签到
function ServerFrameMgr:doSignDone()
    -- 填充协议
    local pack = CCmd_Data:create(4+33*2+33*2+16*2)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.S_SUB_LOAD_SIGN_OPTAION)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalUserItem.szMachine,33)
    pack:pushstring(GlobalData.MyIP,16)
...
--执行连续签到
function ServerFrameMgr:takeContinueSign(cbIndex)
    -- 协议字段
    -- 填充协议
    local pack = CCmd_Data:create(4+1+33*2+16*2)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.S_SUB_LOAD_SIGN_CONTINUE)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushbyte(cbIndex)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    self:AddNetEvent(pack)
end
...
--获取可用的充值或者提现渠道
function ServerFrameMgr:getPayChannel()
    -- 协议字段
    -- 填充协议
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.SUB_MB_GetPayChannel)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end

--获取用户状态
function ServerFrameMgr:AlmzGetUserStatus()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_AlmzGetUserStatus)
    -- 填充字段
    pack:pushdword(GlobalUserItem.dwUserID) -- dwUserID
    self:AddNetEvent(pack)
end

--领取vip救济金奖励
function ServerFrameMgr:AlmzTakeReward()
    local pack = CCmd_Data:create(168)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_AlmzTakeReward)
    -- 填充字段
    pack:pushdword(GlobalUserItem.dwUserID) -- dwUserID
    pack:pushstring(GlobalUserItem.szDynamicPass,33) -- szDynamicPass
    pack:pushstring(GlobalData.MyIP,16) -- szClientIP
    pack:pushstring(GlobalUserItem.szMachine,33) -- szMachineID
    self:AddNetEvent(pack)
end

--获得救济金vip的配置
function ServerFrameMgr:CMD_MB_AlmzLoadConfig()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_AlmzLoadConfig)
    -- 填充字段
    pack:pushdword(GlobalUserItem.dwUserID) -- dwUserID
    self:AddNetEvent(pack)
end
