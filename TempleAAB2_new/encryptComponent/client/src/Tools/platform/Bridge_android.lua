...
function Bridge_android.getClipboardText(callback)
    if GETCLIPBOARDTEXT then
        local args = {callback }
        local sigs = "(I)V"
        local ok,ret  = luaj.callStaticMethod(BridgeClassName.BRIDGE_CLASS_ACTIVITY,FunctionMapping("getClipboardText"),args,sigs)
        if not ok then
            local msg = "luaj error:" .. ret
            print(msg)  
            return 0, msg   
...
--平台分享
function Bridge_android.ShareTelegram(link)
    local args = {link}
    local sigs = "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V"    
    local ok,ret = luaj.callStaticMethod(BridgeClassName.BRIDGE_CLASS_ACTIVITY,FunctionMapping("ShareTelegram"),args,sigs)
    if not ok then
        showToast("mobShare call fail")
        return false
    else
...
function Bridge_android.threeLogEvent(ptype,money)  -- ptype 1:"guestLogon" 2:"facebookLogon" 3:"googleLogon" 4:"open" 5:"register" 6:"purchase"
    print("================threeLogEvent,ptype = ",ptype)
    ...
    if threeData.afsdk == true then
        ...
        local data = {
            event_type = FunctionAfLogName(ptype),
    ...
    if threeData.adjustsdk == true then
        ...
        local data = {
            event_type = FunctionADLogName(ptype),
