...
local OneGame = class("OneGame")

--进入游戏
function OneGame:EnterGame(pID,pExtraParams,cbGameMode)
    --切换竖版
    ...
    GlobalUserItem.roomMark = tonumber(pID)*1000+110+1
    ...
    webView:setOnShouldStartLoading(function(sender,url)
	    ...
	    if target == cc.PLATFORM_OS_WINDOWS or target == cc.PLATFORM_OS_IPHONE or target == cc.PLATFORM_OS_IPAD then
            if url == "http://close.api.command/" then
                self.webView:removeFromParent()
                self.webView = nil
                G_GameFrame:StandUp()
    ...
    webView:reload()
    self:SetOneGameStatus(true)
end

function OneGame:GetOneGameStatus()
    return self.OneGameStatus
end

function OneGame:SetOneGameStatus(pStatus)
    self.OneGameStatus = pStatus
end
