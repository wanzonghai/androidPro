...
//֪ͨ
bool CClientKernel::OnMessageHandler(int nHandler,WORD wMain,WORD wSub,std::string fileName)
{
	bool result = false;
	do
	{
		if(nHandler != 0)
		{
			...
			try {
				if (lua_isfunction(tolua_S, -1))
				{
					...
					CCLOG("OnMessageHandler-luacallback-handler-false:%d",nHandler);
				}
			}
			catch(...) {
				CCLOG("[ERROR]OnMessageHandler-luacallback-handler:%d  cmd:[%hu,%hu]",nHandler, wMain, wSub);
			}
		}
	}while(false);
	return result == 0;
	...
	do
	{
		...
		try {
			if(nHandler != 0 && pCmdData != NULL)
			{
				...
				CCLOG("OnCallLuaSocketCallBack-luacallback-handler-false:%d",nHandler);
			}
		}
		catch(...) {
			CCLOG("[ERROR]OnCallLuaSocketCallBack-luacallback-handler:%d  cmd:[%hu,%hu]",nHandler, pCmdData->GetMainCmd(), pCmdData->GetSubCmd());
		}
	}while(false);
	return result == 0;
}
...
void CClientKernel::OnMessageRespon(int nLuaFunID, TNetPacket* message)
{
	...
	if(message)
	{
		...
		case MSG_HTTP_DOWN:
			if (message->Length > 0)
			{
				OnMessageHandler(nLuaFunID, message->MCmd, message->SCmd, ((char *)message->Data));
			}
			else {
