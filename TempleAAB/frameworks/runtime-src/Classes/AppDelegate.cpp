...
int System_GetIp(const std::string cmd, std::string &output)
{
	FILE * fp;
	int res = -1;
#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 
...
static int toLua_AppDelegate_downFileAsync(lua_State* tolua_S)
{
	...
	if ( argc == 4 )
	{
		...
		if (handler != 0)
		{
			CDownAssetManager::DownFile(szUrl,szSaveName,szSavePath,handler,10);
	...
	} else if ( argc == 5 )
	{
		...
		const char* szSavePath = lua_tostring(tolua_S,3);
		int handler = toluafix_ref_function(tolua_S,4,0);
		int timeout = toluafix_ref_function(tolua_S,5,0);
		if (handler != 0)
		{
			CDownAssetManager::DownFile(szUrl,szSaveName,szSavePath,handler,timeout);
			lua_pushboolean(tolua_S, 1);
			return 1;
		}
		else
...
static int toLua_AppDelegate_CCGetClientIp(lua_State* tolua_S) {
#if CC_TARGET_PLATFORM != CC_PLATFORM_IOS && CC_TARGET_PLATFORM != CC_PLATFORM_MAC
	std::string result;
	std::string cmd;
	auto argc = lua_gettop(tolua_S);
	if (argc == 1) {
		const char* tmp = lua_tostring(tolua_S, 1);
		if (tmp && *tmp) {
			cmd = tmp;
		}
		else {
			CCLOG("CCGetClientIp  Args= NULL ----------------!!!!!!!!!!!!!!!!!");
	...
	int ret = System_GetIp(cmd.c_str(),result);
	if (ret == 0 && result.length() > 0) {
		lua_pushstring(tolua_S, result.c_str());
	}else {
		lua_pushstring(tolua_S, "127.0.0.1");
	}
	return 1;
#endif
}
...
static int register_all_packages()
{
	...
	lua_register(tolua_S, "CCGetClientIp", toLua_AppDelegate_CCGetClientIp);
#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
