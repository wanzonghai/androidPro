...
function Game1ViewLayer:preAddGameItem()
	local itemInfo = {3,2,3,1,5,1,1,0,2,3,1,3,5,4,8}
	for i = 1, 15 do
...
function Game1ViewLayer:sendAutoMsg()
...
--置灰按钮
function Game1ViewLayer:grayButton(bool)
	self._Button_Start:setTouchEnabled(bool)
	self._Button_Start:setBright(bool)  
	self._Button_Sub:setTouchEnabled(bool)
	self._Button_Sub:setBright(bool) 
	self._Button_Add:setTouchEnabled(bool)
	self._Button_Add:setBright(bool) 
end
...
function Game1ViewLayer:setAutoStart( bisShow )
	...
	local isEnable = not bisShow
	self:enableGame1Btn(isEnable)
end

function Game1ViewLayer:enableGame1Btn( isEnable )
	local Button_Start = self._csbNode:getChildByName("Button_Start");
	if isEnable then
		if self._scene:getGameMode() == 2   then			--如果是正在转动

		else
			self:grayButton(isEnable)
		end
	else
		self:grayButton(isEnable)
	end
...
function Game1ViewLayer:checkBigWin()
    local m_lYafen  = self._scene.m_lBetScore[self._scene.m_bYafenIndex]        --压分
    local m_lTotalYafen  = m_lYafen*self._scene.m_lYaxian         --总压分
    local beishu = self._scene.m_lGetCoins/m_lTotalYafen 
    local index = 1
	local lGetScoreStep = self._scene.lGetScoreStep[1]
    if beishu < lGetScoreStep[1] then return end
	...
	end,beishu,self._scene.m_lGetCoins,lGetScoreStep)
    --self:showBinWinPanel(index,self._scene.m_lGetCoins)
end
...
function Game1ViewLayer:showRewardTipNode(_call, _rate, _nums,lGetScoreStep)
    ...
    local _rewardTipNode = RewardTipNode:create(data):addTo(self._csbNode, 10000)
