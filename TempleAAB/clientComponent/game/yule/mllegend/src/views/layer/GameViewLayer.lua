...
--    end
    ...
    local _itemInfo = {{3,4,7,6,4,7},{1,0,3,5,1,0},{6,5,0,3,9,3}}
    self.m_ScrollLayer:setGameSceneData(_itemInfo)
...
function GameViewLayer:onButtonClickedEvent(tag, ref)
    ...
    elseif tag == TAG_ENUM.TAG_START_BTN then   -- 开始游戏        
        if self._scene.m_lTotalYafen > self._scene.m_lScore + self._scene.m_lGetCoins and self._scene.m_bFreetime == false then
            local num = self._scene.m_bFreetime and 1 or 0
        ...
        if self._scene._bReceiveSceneMsg == false or self._scene.m_bIsItemMove == true then
            return ;
        end
...
function GameViewLayer:onGetGameScore()
    ...
    local function callback()
        ...
        if self._scene.m_bIsAuto or self._scene.m_bFreetime or self._scene.m_cbGameStatus == g_var(cmd).SHZ_GAME_SCENE_FREETIME then
            ...
            local beishu = self._scene.m_lWinScore/m_lTotalYafen 
            if beishu < self._scene.lGetScoreStep[1] then 
                self._scene:GameStart()
            else
                local fTime= 3.0
                self:performWithDelay(fTime,function()
                    self._scene:GameStart()
                end)   
            end
        end
    ...
    if _getcoin>0  then
        ...
        self:performWithDelay(0.5,callback)     
...
function GameViewLayer:checkBigWin()
    local m_tStartDate = self._scene.m_tStartDate
    local childitem = m_tStartDate and m_tStartDate.childitem or {}
    local m_cbDeleteTime = self._scene.m_cbDeleteTime
    --m_cbDeleteTime = m_cbDeleteTime + 1
    if m_cbDeleteTime ~= #childitem then return end
    local m_lYafen  = self._scene.m_lBetScore[self._scene.m_bYafenIndex]        --压分
    local m_lTotalYafen  = m_lYafen*self._scene.m_lYaxian         --总压分
    local beishu = self._scene.m_lWinScore/m_lTotalYafen 
    local index = 1
    if beishu < self._scene.lGetScoreStep[1] then return end
    for k = 1,#self._scene.lGetScoreStep do
        if beishu >= self._scene.lGetScoreStep[k] then
            index = k
        end
    end
    ...
    self:showRewardTipNode(function() 
        ...
        --     self._scene:GameStart()
        -- end
    end,beishu,self._scene.m_lWinScore,self._scene.lGetScoreStep)
 --   self:showBinWinPanel(index,self._scene.m_lWinScore)
end
