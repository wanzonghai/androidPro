...
function GameItem:beginMove( deyTime , JiaSu_YanShi,endCallBack )
	if not self._isScroll then
		if self.m_pSprite  then
			self.m_pSprite:stopAllActions()
			self.m_pSprite:removeFromParent()
		...
		self.m_pSprite:setTexture("common_MoveImg.png")
		self.m_pSprite:setPosition(cc.p(105,685))
	end
...
function GameItem:stopScroll()
	if self.m_pSprite and (not tolua.isnull(self.m_pSprite))  then
