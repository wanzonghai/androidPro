...
function Game1ViewLayer:Button_Gray(Gray)
    ...
    if not Gray then
        self.Button_Start:setTouchEnabled(Gray)
        self.Button_Start:setBright(Gray)
    else
        self.Button_Start:setTouchEnabled(not self._scene.m_bIsAuto)
        self.Button_Start:setBright(not self._scene.m_bIsAuto)
    end
end

function Game1ViewLayer:createParticle(indexName)
    if indexName <= 1 then return end
    local rootPath = "BigWin/".."za_lb_jbfzmpdl"
    local animation = sp.SkeletonAnimation:create(rootPath..".json",rootPath..".atlas",1)
    self.Panel_BigWin:addChild(animation)
    animation:setAnimation(0,"a"..(indexName - 1),true)
    animation:setPosition(667,375)
end

function Game1ViewLayer:showBigWin(m_lGetCoins)
    local lGetScoreStep = self._scene.lGetScoreStep[1]
    if m_lGetCoins == 0 then
        return
    end

    local beishu = m_lGetCoins / self._scene:getTotalYaFen()
    if beishu < lGetScoreStep[1] then
        return
    end
    local index = 0
    for k = 1,#lGetScoreStep do
        if beishu >= lGetScoreStep[k] then
            index = k
        end
    end
    ...
    end,beishu,m_lGetCoins,lGetScoreStep)
    --self:showBinWinPanel(index,m_lGetCoins)
end

--预先转动
function Game1ViewLayer:preStart()
    if self._scene.m_lTotalYafen > self._scene.m_lCoins  then
        -- 提示游戏币不足
    ...
    for i = 1, 15 do
        ...
        if pItemLast then
            pItemLast:startScroll()
        end
    end
end

function Game1ViewLayer:stopScroll()
    for i = 1, 15 do
        local posx = math.ceil(i / 3)
        local posy =(i - 1) % 3 + 1
        local nodeStr = string.format("Node_%d_%d", posx - 1, posy - 1)
...
function Game1ViewLayer:game1Begin()
    ...
    local zhengchangshijian1 = 0.08
    local zhengchangdianchengshijian = 0.05
    time = 1.6
    -- 正常播放滚动效果时间
    if self._scene.m_bIsAuto or self.MianFeiMu then
        zhengchangshijian1 = 0.06
        -- 0.025
        zhengchangdianchengshijian = 0.05
        -- 0.05
        time = 1
    end
            ...
            if pItem then
                ...
                if Index >= 3 or Index2 >= 3 then
                    ...
                    if i < 7 and True1 == 1 then
                        self:Roll_Game_List(zhengchangshijian1 + (i - 1) * zhengchangdianchengshijian, pItem, node, 0, i)
                        if i == 6 then
                            BeginTimeView_One = zhengchangshijian1 + (i - 1) * zhengchangdianchengshijian
                            -- 记录第一个滚动加速时间产生时间
                    ...
                    elseif i < 13 and True1 == 1 then
                        ...
                        if True1 == 2 then
                            EndTimeView_One = zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.2)
                            -- 记录第一个滚动加速时间结束时间
                            self:RunEnd_Action(node, 1, JiaSu_YanShi)
                            self:Roll_Game_List(zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.2), pItem, node, JiaSu_YanShi, i)

                            if i == 6 then
                                True1 = 3
                                BeginTimeView_Two = zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.2)
                                -- 记录第二个滚动加速时间产生时间
                        ...
                        elseif True1 == 3 then
                            ...
                            EndTimeView_Two = zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.4)
                            -- 记录第二个滚动加速时间结束时间
                            self:RunEnd_Action(node, 2, JiaSu_YanShi)

                            self:Roll_Game_List(zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.4), pItem, node, JiaSu_YanShi, i)

                            if i == 9 then
                                BeginTimeView_Three = zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.4)
                                -- 记录第三个滚动加速时间产生时间
                        ...
                        elseif True1 == 4 then
                            ...
                            EndTimeView_Three = zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.6)
                            -- 记录第个滚动加速时间结束时间
                            self:RunEnd_Action(node, 3, JiaSu_YanShi)
                            self:Run_Action(node, 3, JiaSu_YanShi)
                            self:Roll_Game_List(zhengchangshijian1 + (i - 1) *(zhengchangdianchengshijian + 0.6), pItem, node, JiaSu_YanShi, i)
                            if i == 12 then
                                True1 = 5
                            end
                        end
    ...
    if Index > 2 or Index2 > 2 then
        if EndTimeView_One ~= 0 then
            time = EndTimeView_One + 0.2
        end
        if EndTimeView_Two ~= 0 then
            time = EndTimeView_Two + 0.2
        end
        if EndTimeView_Three ~= 0 then
            time = EndTimeView_Three + 0.2
        end
    end
...
function Game1ViewLayer:Roll_Game_List(Roll, pItem1, node1, True1, i)
    ...
    cc.Sequence:create(
    cc.CallFunc:create( function()
        pItem1:beginMove(Roll, True1,function() 
            if (i % 3) == 0 then
                g_ExternalFun.playEffect("sound_res/normal_stop_C64kbps.mp3")
            end

            if i == 15 then
                self:showBigWin(self._scene.m_lGetCoins)
...
--            self:SetNumber_And_Meney(1, self.m_textScore)
    ...
    if g_var(cmd).SHZ_GAME_SCENE_THREE == self._scene.m_cbGameStatus then
        ...
        elseif self._scene.m_bIsAuto == true  then
            -- 自动游戏中
            local RewardTipNode = self._csbNode:getChildByName("RewardTipNode")
...
function Game1ViewLayer:autoSendMsg()
    self.TeShuTuBiao = false
...
function Game1ViewLayer:JINBITEXIAO(jinbiname, liziname, jinbi, effect , pos)
    ...
    -- 高额金币播放
    local Boss = g_ExternalFun.loadCSB(GameViewLayer.RES_PATH .. "Game1_Terrace/Small/Specia_Effects/User_Operation/" .. jinbiname .. ".csb", self._csbNode);
...
function Game1ViewLayer:onBtnExit()
...
function Game1ViewLayer:setAutoStart(bisShow)
     ...
     if not self._scene.m_bIsItemMove then
...
--奖励提示界面
function Game1ViewLayer:showRewardTipNode(_call, _rate, _nums,lGetScoreStep)
    self:hideRewardTipNode()
    ...
    data._auto = self._scene.m_bIsAuto
    local _rewardTipNode = RewardTipNode:create(data):addTo(self._csbNode, 100000)
