...
function GameViewLayer:ctor(rootNode)
    ...
    cc.SimpleAudioEngine:getInstance():preloadEffect("sounds/YOU_WIN.mp3")
    cc.SimpleAudioEngine:getInstance():preloadEffect("sounds/BIG_WIN.mp3")
    cc.SimpleAudioEngine:getInstance():preloadEffect("sounds/MEGA_WIN.mp3")
    cc.SimpleAudioEngine:getInstance():preloadEffect("sounds/SUPER_WIN.mp3")
...
function GameViewLayer:initCanvas()
    ...
    self.mm_btn_auto:onClicked(function() 
        ...
        if self.subGoldCount > self.userGold then
            --提示游戏币不足
...
function GameViewLayer:checkAuto()
...
function GameViewLayer:setBtnStopStatus(isShow)
    self.mm_btn_stop:setVisible(false) 
...
function GameViewLayer:sendStart()
    ...
    self:runAllAction(logic.gameData.result_icons,10)
end
--游戏开始返回
function GameViewLayer:betResult(data)
    ...
    self:runAllAction(data.result_icons,2)
...
function GameViewLayer:createBigwin(_callback,goldCount)
    ...
    local betScore = self.betArray[self.curBetIndex] * self.betScore
    local beishu = goldCount / betScore
    ...
    self:showRewardTipNode(func,beishu,goldCount,lGetScoreStep)
...
function GameViewLayer:initScrollItem()
    ...
    local result_icons = {{1,2,3,4,5},{2,1,5,4,3},{3,2,4,5,1}}
    self.turnedAround:initServerData(result_icons)
    self.mm_Panel_icon:show()
end
--旋转动画
function GameViewLayer:runAllAction(nFruitAreaDistri,round)
