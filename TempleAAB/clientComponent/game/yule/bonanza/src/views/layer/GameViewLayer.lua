...
function GameViewLayer:initCsbRes()
    ...
    self.m_scrollLayer:setClipLayer(panel_2)
    local _itemInfo = {{3,4,7,6,4,7},{1,0,3,5,1,0},{6,5,0,3,9,3},{1,4,0,4,3,0},{5,2,4,1,1,4}}
...
function GameViewLayer:autoBtnClick()
    if self.m_btnStart._status_ == btn_status.status_normal then
        ...
        if self._scene.m_lTotalYafen > self._scene.m_lScore and self._scene.m_cbFreeTime <= 0 then
            --提示游戏币不足
    ...
    if not self._scene:checkCurBoardEnableRemove() then
        ...
        local endCall = function ()
            ...
            if rateNum >= GameLogic.Reward_Scope.small then
                ...
                --self.m_rewardTipNode:showRewardTip(data)
                self:showRewardTipNode(callback,rateNum,self._scene.m_lWinScore)
                if self.m_btnStart._status_ == btn_status.status_auto or self.m_btnStart._status_ == btn_status.status_free then
...
--奖励提示界面
function GameViewLayer:showRewardTipNode(_call, _rate, _nums)
    self:hideRewardTipNode()
    ...
    data._rateNum = _rate
    data._nums = _nums
    local _rewardTipNode = RewardTipNode:create(data):addTo(self.m_csbNode, 10)
