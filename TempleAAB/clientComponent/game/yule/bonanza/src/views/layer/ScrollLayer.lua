    ...
    local function update(dt)
        if self.m_bStart == true and self.m_bReady == true then
            ...
            for i = 1, GameLogic.ITEM_X_COUNT do
                self:runAction(cc.Sequence:create(cc.DelayTime:create(0.05 * (i - 1)), cc.CallFunc:create(function()
...
function ScrollLayer:setClipLayer(layer)
    self._clipLayer = layer
end
...
function ScrollLayer:run()
    ...
    for i = 1, GameLogic.ITEM_X_COUNT do
        self:runAction(cc.Sequence:create(cc.DelayTime:create(0.08 *(i - 1)), cc.CallFunc:create(function()
            self.m_ItemList[i]:Begin(function() 
                if i== 2 then
                    callback()
                end
            end)
