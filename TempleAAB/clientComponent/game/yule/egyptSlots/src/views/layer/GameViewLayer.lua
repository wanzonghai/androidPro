...
local RewardTipNode = appdf.req(module_pre .. ".views.layer.RewardTipNode")
local GameSetting = appdf.req(module_pre .. ".views.layer.GameSetting")
...
function Game1ViewLayer:onExit()
    ...
    cc.SpriteFrameCache:getInstance():removeSpriteFramesFromFile("game/yule/egyptSlots/res/game_setting.plist")
end
...
function Game1ViewLayer:addPath( )
    ...
    cc.SpriteFrameCache:getInstance():addSpriteFrames("game/yule/egyptSlots/res/game_setting.plist")
end
...
function Game1ViewLayer:initUI()
    ...
    self.Button_YaMan:setVisible(false)

    --自动
    self.Button_Auto = ccui.Button:create('egitoslot/btn_auto.png',"","",ccui.TextureResType.localType)--ccui.TextureResType.plistType
    self.Button_Auto:setPosition(self.Button_YaMan:getPosition())
    csbNode:getChildByName("panel_bottom"):addChild(self.Button_Auto)
	...
	------
    local img_goldbg = csbNode:getChildByName("panel_top"):getChildByName("img_goldbg")
    img_goldbg:loadTexture("game_common_jinbixinxi_di.png",ccui.TextureResType.plistType)
	--游戏币
	self.m_textScore = img_goldbg:getChildByName("gold")
   ...
   -- 场景状态
    ...
    self.GameSetting = GameSetting:create(self)
    self:addChild(self.GameSetting)
end

--游戏1动画开始
function Game1ViewLayer:game1Begin()
    self:hideRewardTipNode()
    self.m_textTips:setVisible(false)
    ...
    for i=1,5 do
        ...
        local moveby6 = cc.MoveBy:create(0.6+delayTime[i]*2, cc.p(0, -1705))--[[-205*9+140 ))]]
        local moveby7 = cc.MoveBy:create(0.09, cc.p(0, -70))
        local moveby8 = cc.MoveBy:create(0.09, cc.p(0, -50))
        local moveby9 = cc.MoveBy:create(0.05, cc.p(0, -30))
        local moveby10 = cc.MoveBy:create(0.05, cc.p(0, -10))
        local moveby11 = cc.MoveBy:create(0.1, cc.p(0, 20))
        self.m_fruitNode[i]:runAction( cc.Sequence:create(moveby6,moveby7,moveby8,moveby9,moveby10,moveby11,
             cc.CallFunc:create(function() 
                self.m_fruitNode[i].bEndAction = true 
                ExternalFun.playSoundEffect("Reel_Scrolling.mp3")
            end)))
    end
...
--游戏1动画开始
function Game1ViewLayer:game1BeginBefore()
    self:hideFreeWinNode()
    self.m_textTips:setVisible(false)
    self.Button_Add:setEnabled(false)
    self.Button_Sub:setEnabled(false)
    self.Button_YaMan:setEnabled(false)
    ...
    for i=1,5 do
        ...
        local moveby7 = cc.CallFunc:create(function()
            self.m_fruitNode[i]:setPositionY(self.m_fruitNodePosY[i]+1845)--205*9)
                  end)
        local moveby8 = cc.MoveBy:create(0.6, cc.p(0, -1705))--[[-205*9+140 ))]]
        local moveby9 = cc.CallFunc:create(function()
                    self.m_fruitNode[i]:setPositionY(self.m_fruitNodePosY[i]+1845)--205*9)
                end)
        local moveby10 = cc.MoveBy:create(0.6, cc.p(0, -1705))--[[-205*9+140 ))]]
        local moveby11 = cc.CallFunc:create(function()
            self.m_fruitNode[i]:setPositionY(self.m_fruitNodePosY[i]+1845)--205*9)
                    end)
...
function Game1ViewLayer:game1Result()
    ...
    local rateNum = self._scene.m_lGetCoins / self._scene.m_lTotalYafen--self._scene.m_lGetCoins / self._scene.m_lTotalYafen --当局中奖倍数
    if self._scene.m_lWinScore > 0 then --self._scene.m_lWinScore > 0 then
        self:showFreeWinNode(self._scene.m_lWinScore)
    else
    ...
    if self._scene.m_lGetCoins > 0 then
       ...
       nodeGold:runAction(cc.Sequence:create(cc.DelayTime:create(0.4),cc.CallFunc:create(function ()
        ...
        local fTime = 0.5
        if self._scene.m_lWinScore > 0 then
            fTime = 1
            local rateNum = self._scene.m_lWinScore / self._scene.m_lTotalYafen --当局中奖倍数
            tlog('rateNum is ', rateNum, GameLogic.Reward_Scope.small)
            if rateNum >= GameLogic.Reward_Scope.small then
                fTime= 3.0
                -- if rateNum >= GameLogic.Reward_Scope.superbig then
                --     fTime = 8
                -- elseif rateNum >= GameLogic.Reward_Scope.big then
                --     fTime = 6.5
                -- elseif rateNum >= GameLogic.Reward_Scope.middle then
                --     fTime = 5
                -- end
            end
        end
...
function Game1ViewLayer:setAutoStart( bisShow )
    ...
    if bisShow == true then
        self.Button_Auto:loadTextureNormal('egitoslot/btn_auto_select.png')
    else
        self.Button_Auto:loadTextureNormal('egitoslot/btn_auto.png')
    end
end
...
--奖励提示界面
function Game1ViewLayer:showRewardTipNode(_call, _rate, _nums)
    self:hideRewardTipNode()
    tlog('GameViewLayer:showRewardTipNode ', _rate, _nums)
    local data = {}
    data._callBack = _call
    data._rateNum = _rate
    data._nums = _nums
    local _rewardTipNode = RewardTipNode:create(data):addTo(self._csbNode, 10)
    _rewardTipNode:setPosition(self._csbNode:getContentSize().width * 0.5, self._csbNode:getContentSize().height * 0.5)
    _rewardTipNode:setName("RewardTipNode")
end

function Game1ViewLayer:hideRewardTipNode()
    if self._csbNode:getChildByName("RewardTipNode") then
        self._csbNode:removeChildByName("RewardTipNode")
    end
end


--免费结算界面
function Game1ViewLayer:showFreeWinNode(_nums)
    if self._csbNode:getChildByName("_freeWinNode") then
        self._csbNode:removeChildByName("_freeWinNode")
    end

    local _freeWinNode = display.newLayer(cc.c4b(0, 0, 0, 125))    
    _freeWinNode:setName("_freeWinNode")
    self._csbNode:addChild(_freeWinNode,10)

    local youwin = sp.SkeletonAnimation:create(GameViewLayer.RES_PATH .. "spine/youwin.json",GameViewLayer.RES_PATH .. "spine/youwin.atlas", 1)
    youwin:setPosition(self._csbNode:getContentSize().width * 0.5, self._csbNode:getContentSize().height * 0.5+80)
    youwin:setAnimation(0, "animation", true)      
    _freeWinNode:addChild(youwin,10)
    youwin:setScale(0.5)

    local scale1 = cc.ScaleTo:create(0.11,1.1)
    local scale2 = cc.ScaleTo:create(0.11,1.0)
    youwin:runAction(cc.Sequence:create(scale1,scale2))


    local freeWinText = ccui.TextBMFont:create()--ccui.TextBMFont:create("123",GameViewLayer.RES_PATH .. "fonts/jnh_bigwin_num.fnt",36) 
    :setFntFile(GameViewLayer.RES_PATH .. "fonts/img_num.fnt")
    local serverKind = G_GameFrame:getServerKind()
    local formatMoney = g_format:formatNumber(_nums,g_format.fType.standard,serverKind)
    freeWinText:setString(formatMoney)
    freeWinText:setPosition(youwin:getContentSize().width/2,youwin:getContentSize().height/2-140)
    youwin:addChild(freeWinText,10)

    _freeWinNode:runAction(cc.Sequence:create(cc.DelayTime:create(2.5),cc.CallFunc:create(function()
            _freeWinNode:removeFromParent(true)
        end)))
end

function Game1ViewLayer:hideFreeWinNode()
    if self._csbNode:getChildByName("_freeWinNode") then
        self._csbNode:removeChildByName("_freeWinNode")
    end
end
