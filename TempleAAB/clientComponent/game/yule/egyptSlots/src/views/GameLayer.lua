...
function GameLayer:OnInitGameEngine()
    ...
    self.m_llFreeScore          = 0         --免费获得分
    self.m_lWinScore            = 0         --免费总赢分
    self.m_bFree            = false     --是否免费
...
function GameLayer:onEventGameSceneFree(buffer)    --空闲 
    ...
    if cmd_table.lGetScoreStep then
        GameLogic.Reward_Scope.small = cmd_table.lGetScoreStep[1][1] or GameLogic.Reward_Scope.small
        GameLogic.Reward_Scope.middle = cmd_table.lGetScoreStep[1][2] or GameLogic.Reward_Scope.middle
        GameLogic.Reward_Scope.big = cmd_table.lGetScoreStep[1][3] or GameLogic.Reward_Scope.big
        GameLogic.Reward_Scope.superbig = cmd_table.lGetScoreStep[1][4] or GameLogic.Reward_Scope.superbig
    end
...
function GameLayer:onGame1Start(dataBuffer) --游戏开始
    ...
    if self.m_FreeTime > 0 then
        ...
        if self.m_bFree == true then
            self.m_bFree = false
            self.m_lWinScore = self.m_llFreeScore + self.m_lGetCoins
        end
        ...
        elseif self:getGameMode() == GAME_STATE.GAME_STATE_WAITTING_GAME2 or self:getGameMode() == GAME_STATE.GAME_STATE_END  then
            ...
            self._gameView.m_textTips:setString("Boa sorte!")
             if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then
                ...
                self._gameView:updateStartButtonState(true)
                if self.showJumpVipDialog then
                    self:showJumpVipDialog(function()
                        -- body
                        self:onExitTable()
                    end)
                    return
            ...
            self:setGameMode(1)
            --self._gameView:game1BeginBefore()
        end
    end
    if self:getGameMode() == GAME_STATE.GAME_STATE_MOVING then
        self._gameView:game1End()
...
function GameLayer:onAddScore()
    self._gameView:onHideTopMenu()
    if GlobalUserItem.LaspPayTime and GlobalUserItem.LaspPayTime <= 0 then
        self:showTipsChangeNote(function()
            -- body
            self:onExitTable()
        end)
        return
