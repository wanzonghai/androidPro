...
function CarnivalScrollLayer:startRun(_speedFactor, _freeStatus)
    self:stopScheduleEvent()
    for i = 1, GameLogic.ITEM_X_COUNT do
        ...
        self.m_itemList[i]:setStartRunData(speed, GameLogic.TIME_NETWORK_TIME, rollConfig[i], endCall)
    end
    self.m_scheduleUpdate = g_scheduler:scheduleScriptFunc(handler(self, self.rollingUpdate), 0, false)
end

function CarnivalScrollLayer:startRunWithNetData(_speedFactor, _bonusIndex, _freeStatus, _bonusCall)
    tlog('CarnivalScrollLayer:startRun ',_bonusIndex)
    --self:stopScheduleEvent()
    ...
    for i = 1, GameLogic.ITEM_X_COUNT do
        ...
        self.m_itemList[i]:setCurRollTime( maxRollTime, endCall)
    end
end
