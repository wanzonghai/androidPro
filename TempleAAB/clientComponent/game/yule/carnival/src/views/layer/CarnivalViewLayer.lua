...
local CarnivalRewardTipNode = appdf.req(module_pre .. ".views.layer.RewardTipNode")
...
function CarnivalViewLayer:initCsbRes()
    ...
    local _itemInfo = {{0,0,4,0,11},{1,10,3,6,1},{1,1,4,5,0},{5,9,7,4,2}}
...
function CarnivalViewLayer:initBtnShow(_parentNode)
    ...
    -- self.m_btnList = _topNode:getChildByName("Image_set")
...
--播放背景音乐
function CarnivalViewLayer:onPlayMusic()
    if GlobalUserItem.bVoiceAble then
        if self.m_freeGameBg:isVisible() then
            self:playGamebgMusic(2)
        else
...
function CarnivalViewLayer:updataBtnEnable()
    ...
    self.m_btnStart:setEnabled(normalBtnEnable)
...
function CarnivalViewLayer:start()
    if self._scene.m_lTotalYafen > self._scene.m_lScore then
        showToast(g_language:getString('game_tip_no_money'))
        --处理auto按钮回退显示成开始按钮
        self:reflushBtnStatusByStopAuto()
...
function CarnivalViewLayer:gameBegin()
    ...
    --rollLayer:startRun(self.m_speedFactor, self._scene.m_bonusStartIndex, self.m_freeGameBg:isVisible(), bonusCall)
    rollLayer:startRunWithNetData(self.m_speedFactor, self._scene.m_bonusStartIndex, self.m_freeGameBg:isVisible(), bonusCall)
...
function CarnivalViewLayer:hideRewardTipNode()
    self:removeChildByName("CarnivalRewardTipNode")
end
