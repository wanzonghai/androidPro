...
function GameViewLayer:onStartGameClick(target)
    ...
    -- self.mm_btn_green:setEnabled(false)
    -- self.mm_btn_green:setBright(false)
    -- self.mm_btn_yellow:setEnabled(false)
    -- self.mm_btn_yellow:setBright(false)
    -- self.mm_btn_red:setEnabled(false)
    -- self.mm_btn_red:setBright(false)

    if self:checkAutoStatus() == false then 
        if self._scene.showJumpVipDialog then
            self._scene:showJumpVipDialog(function()
                -- body
                self:onExitClick()
            end)
        end
    ...
    self:stopActionByTag(0x0001)   
    local action = cc.Sequence:create(cc.DelayTime:create(0.6),cc.CallFunc:create(function() 
        self:showLoading()
    end)) 
    action:setTag(0x0001)
...
function GameViewLayer:initCanvas()
    ...
    self.mm_btn_pins:onClicked(
        function()
            print("mm_btn_pins")
            local isShow = self.mm_Image_pinsbg:isVisible()
            self.mm_Image_pinsbg:setVisible(not isShow)
            self.mm_btn_pins:loadTextureNormal(isShow and "game/yule/Plinko/res/GUI/meun/bt_pinos.png" or "game/yule/Plinko/res/GUI/meun/bt_pinos01.png")
            self.mm_btn_pins:loadTexturePressed(isShow and "game/yule/Plinko/res/GUI/meun/bt_pinos.png" or "game/yule/Plinko/res/GUI/meun/bt_pinos01.png")
            for i = 1, 3 do
    ...
    self.m_centerNode = g_ExternalFun.loadCSB("UI/scoreAction.csb", self.mm_centerMeNode)
    g_ExternalFun.loadChildrenHandler(self.m_centerNode, self.m_centerNode)
    self.m_centerNode:setPosition(cc.p(0,0))
    self.m_centerNode:hide()

    self:initBigWinAction()
...
function GameViewLayer:initBigWinAction()
    self.bg = display.newLayer(cc.c4b(0,0,0,125))
    self.bg:setPosition(-display.cx, -display.cy)
    self.mm_centerMeNode:addChild(self.bg)

    self.jiesuan1 = sp.SkeletonAnimation:create("spine/jiesuan1.json","spine/jiesuan1.atlas", 1)
    self.jiesuan1:setPosition(0, 60)
    self.jiesuan1:setAnimation(0, "daiji", true)
    self.mm_centerMeNode:addChild(self.jiesuan1,2)

    self.BigWin = cc.CSLoader:createNode("UI/BigWin.csb")
    self.BigWin:setPosition(0, -60)
    self.mm_centerMeNode:addChild(self.BigWin,2)
    --self.BigWinTimeLine = cc.CSLoader:createTimeline("UI/BigWin.csb")

    self.Image_coin = self.BigWin:getChildByName("Image_coin")
    self.BigWinText = self.BigWin:getChildByName("Text_1")

    self.jiesuan2 = sp.SkeletonAnimation:create("spine/jiesuan2.json","spine/jiesuan2.atlas", 1)
    self.jiesuan2:setPosition(0, 60)
    self.jiesuan2:setAnimation(0, "daiji", true)
    self.mm_centerMeNode:addChild(self.jiesuan2,1)

    self.bg:setVisible(false)
    self.jiesuan1:setVisible(false)
    self.BigWin:setVisible(false)
    self.jiesuan2:setVisible(false)
end

--12、14、16线暂分别写死为达到25倍、49倍、61倍触发
function GameViewLayer:checkBigWin(gameData)
    local cbLineType = gameData.cbLineType --12 14 16
    local cbGameMode = gameData.cbGameMode --绿 蓝 红 color
    local cbItemIndex = gameData.cbItemIndex
    local line = logic.pinsMap[cbLineType+1]
    local odds = logic.odds[line]
    local list = odds[cbGameMode+1]
    local winData = list[cbItemIndex+1]
    local num = tonumber(winData or 0)
    if cbLineType == 0 then
        if num >= 25 then
            return true
        end
    elseif cbLineType == 1 then
        if num >= 49 then
            return true
        end
    elseif cbLineType == 2 then
        if num >= 61 then
            return true
        end
    end
...
function GameViewLayer:showBigWinAction(score)
    self.m_centerNode:setVisible(false)

    self.jiesuan1:setAnimation(0, "ruchang", false)
    self.jiesuan2:setAnimation(0, "ruchang", false)
    self.jiesuan1:addAnimation(0,"daiji",true)
    self.jiesuan2:addAnimation(0,"daiji",true)
    local strScore = "+" .. g_format:formatNumber(score,g_format.fType.standard,serverKind)
    self.BigWinText:setString(strScore)
    local size = self.BigWinText:getContentSize()
    local x = self.BigWinText:getPositionX()
    local y = self.BigWinText:getPositionY()
    self.Image_coin:setPosition(x-size.width/2-35,y)

    self.BigWin:stopAllActions()
    self.BigWin:setScale(0.1)
    local action = cc.Sequence:create(cc.ScaleTo:create(0.2, 1),cc.DelayTime:create(1.3),cc.CallFunc:create(function() 
        self.bg:setVisible(false)
        self.jiesuan1:setVisible(false)
        self.BigWin:setVisible(false)
        self.jiesuan2:setVisible(false)
    end))
    self.BigWin:runAction(action)

    self.bg:setVisible(true)
    self.jiesuan1:setVisible(true)
    self.BigWin:setVisible(true)
    self.jiesuan2:setVisible(true)
end
...
function GameViewLayer:onBetBatchClick()
    if not self:getParentNode():onAddScore() then
        return
    end
    if self.m_isAutoBet then return end 
...
function GameViewLayer:showLoading()
    if not self._skeletonNode then
        local skeletonNode = sp.SkeletonAnimation:create("spine/internetlost_action.json", "spine/internetlost_action.atlas", 1)
        skeletonNode:addAnimation(0, "daiji", true)    
        self._skeletonNode = skeletonNode
        self.mm_centerMeNode:addChild(skeletonNode)
    else
        self._skeletonNode:show()
    end
    self._skeletonNode:stopAllActions()
    local action = cc.Sequence:create(cc.EaseSineOut:create(cc.ScaleTo:create(1/6,1.05)),cc.EaseSineIn:create(cc.ScaleTo:create(1/6,1)))
    self._skeletonNode:setScale(0)
    self._skeletonNode:runAction(action)
end

function GameViewLayer:disShowLoading()
    self:stopActionByTag(0x0001)
    if self._skeletonNode and self._skeletonNode:isVisible() then
        local action = cc.Sequence:create(cc.EaseElasticOut:create(cc.ScaleTo:create(1/6,0)),cc.CallFunc:create(function() 
            self._skeletonNode:hide()
        end))
        self._skeletonNode:stopAllActions()
        self._skeletonNode:runAction(action)
    end
    ...
    if viewID == logic.viewID.me then
        ...
        local updateUserScore = function(data, winData)
            ...
            if gameData.win_score > 0 then
                ...
                self.m_centerNode.mm_BitmapFontLabel_1:setString("+" .. g_format:formatNumber(gameData.win_score,g_format.fType.standard,serverKind))
                self.m_centerNode:runAction(nodeAction)
                self.m_centerNode:show()
                nodeAction:play("animation0", false)

                if self:checkBigWin(gameData) then
                    self:showBigWinAction(gameData.win_score)
                end
            end
        end
