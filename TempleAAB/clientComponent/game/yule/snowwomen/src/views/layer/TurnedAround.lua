...
function TurnedAround:ctor(_node)
	...
	self.speed = 4800 --旋转速度
	self._quickModeSpeed = 2400 --快速模式旋转速度
	self._slowModeSpeed = 4800 --慢速模式旋转速度
...
--设置慢速转圈初始圈数
function TurnedAround:setSlow(slow)
	tlog("TurnedAround:setSpeed ", slow)
	self.slow = slow
end
--设置快速和慢速的旋转速度
...
function TurnedAround:stopRollActionEx()
    ...
    local pos = length
    for i=1,length do
        local dist = self.frameHieght* self.tempQuan[i] 
        self.layers[i]:stopAllActions()
