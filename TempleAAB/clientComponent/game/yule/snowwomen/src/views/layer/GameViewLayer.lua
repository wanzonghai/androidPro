...
function GameViewLayer:gameDataInit()
    ...
    self.betCfg = {} --下载配置
    self.isQuick = false--cc.UserDefault:getInstance():getBoolForKey("snowwomen_quick", false) --是否快速
    self.animtime = 0
    --文本按钮等
...
function GameViewLayer:initCsbRes()
	...
	--分辨率适配
	self.m_csbNode:getChildByName("FN_title"):setPositionX(display.cx+600)
	-- self.m_csbNode:getChildByName("Button_more"):setPositionX(display.cx-770)
	-- self.m_csbNode:getChildByName("sp_btn_list"):setPositionX(display.cx-590)
...
function GameViewLayer:onPlayMusic()
...
function GameViewLayer:onButtonClickedEvent(_sender)
	...
	elseif tag == TAG_ENUM.BT_AUTO then
		if not self.isAuto then
			...
			if totalAdd > self.text_userCoin._curNum then
				--提示游戏币不足
				showToast(g_language:getString('game_tip_no_money'))
				if self:getParentNode().showJumpVipDialog then
					self:getParentNode():showJumpVipDialog(function()
						-- body
						self:onExitGame()
					end)
					return
	...
	elseif tag == TAG_ENUM.BT_JIA then
		if not self:getParentNode():onSubScore() then
			return
		end
		if #self.betCfg > 0 then
			self.betCfgIdx = self.betCfgIdx + 1
...
--旋转动画
function GameViewLayer:runAllAction(round)
	tlog('GameViewLayer:runAllAction')
	self.turnedAround:stopRollActionEx()
	--清除上轮中奖动画
	self:clearSpineBoy()
	...
	self.turnedAround:setSlow(round or 5)
...
function GameViewLayer:startGame()
	...
	self.endData = {}
	self.endData.nFruitAreaDistri = result_icons
	self:runAllAction(10)
	self.btn_start:setTouchEnabled(false)
	self.btn_start:setBright(false)
end
--切换自动开始状态
...
function GameViewLayer:_autoStart()
    ...
    if self.autoCount>0 then
        --自动开始
        if self.isAuto then
			local RewardTipNode = self.m_csbNode:getChildByName("RewardTipNode")
			if not RewardTipNode or tolua.isnull(RewardTipNode) then
				self:autoSendMsg()
			end
        else
...
function GameViewLayer:autoSendMsg()
...
function GameViewLayer:show_bigwin_ani()
	...
	if self:isLuaNodeValid(self.xn_bigwin_ske) then
		self.xn_bigwin_ske:setAnimation(0, "a1", false)
		if("binwin" == self.animName)then

	    elseif "sb_lbjs_BIGWIN/sb_lbjs_BIGWIN" == self.animName or "sb_lbjs_MEGAWIN/sb_lbjs_MEGAWIN" == self.animName then
			tickMgr:delayedCall(handler(self, self.showMegaWinP), 5000)
	    elseif("sb_lbjs_SUPER_WIN/sb_lbjs_MEGAWIN" == self.animName)then
...
function GameViewLayer:show_bigwin_ani_idle()
	...
	if self:isLuaNodeValid(self.xn_bigwin_ske) then
		self.xn_bigwin_ske:setAnimation(0, "a2", true)
		if("binwin" == self.animName)then
...
function GameViewLayer:showBinWinPanel(index)
	...
	if not self:isLuaNodeValid(self.Panel_BigWin) then
	    ...
	    spineBoy:registerSpineEventHandler( function( event )
	    	if event.type == "complete" then
		        if event.animation == "a1" then
		            spineBoy:setAnimation( 0, "a2", true)
				end
		    end
		...
		bigText:setString("0")
	    bigText:setPosition(display.width/2, display.height/2-10)
	    bigText:setTag(21)
	...
	g_ExternalFun.playEffect("sound_res/"..bigWinMp3[index])
...
function GameViewLayer:isPlayBigWinAnim(score)
    ...
    if (self.isChange and self.endScene == g_var(cmd).modeType.eFree)
    	...
    	if beishu < configData.llsmall then
	    elseif beishu >= configData.llsmall and beishu < configData.llmiddle then
	    	isHaveAnim = true
	        time = 8
	    elseif beishu >= configData.llmiddle and beishu < configData.llbig then
	    	isHaveAnim = true
	        time = 10
		elseif beishu >= configData.llbig and beishu < configData.llsuperbig then
	    	isHaveAnim = true
	        time=12
	    elseif beishu >= configData.llsuperbig then
			isHaveAnim = true
	        time=14
		end
    end
...
function GameViewLayer:playGoldSound()
	...
	elseif beishu >= configData.llsmall and beishu < configData.llmiddle then
		isHaveAnim = true
        self.animName = "sb_lbjs_YOU_WIN/sb_lbjs_MEGAWIN"
        self.animtime = 8
		index = 1
		if GlobalUserItem.bSoundAble then
			--self._effSoundHandle = AudioEngine.playEffect("sound_res/Level_09_C64kbps.mp3")
		end
	elseif beishu >= configData.llmiddle and beishu < configData.llbig then
		isHaveAnim = true
        self.animName = "sb_lbjs_BIGWIN/sb_lbjs_BIGWIN"
        self.animtime = 10
		index = 2
		if GlobalUserItem.bSoundAble then
			--self._effSoundHandle = AudioEngine.playEffect("sound_res/Level_13_C64kbps.mp3")
		end
	elseif beishu >= configData.llbig and beishu < configData.llsuperbig then
		isHaveAnim = true
        self.animName = "sb_lbjs_MEGAWIN/sb_lbjs_MEGAWIN"
        self.animtime=12
		index = 3
		if GlobalUserItem.bSoundAble then
			--self._effSoundHandle = AudioEngine.playEffect("sound_res/Level_14_C64kbps.mp3")
		end
	elseif beishu >= configData.llsuperbig then
		isHaveAnim = true
        self.animName = "sb_lbjs_SUPER_WIN/sb_lbjs_MEGAWIN"
        self.animtime=14
		index = 4
		if GlobalUserItem.bSoundAble then
			--self._effSoundHandle = AudioEngine.playEffect("sound_res/Level_14_C64kbps.mp3")
    ...
    if isHaveAnim then
		...
		self.m_bgMusicType = 0
		local configData = self.configData
		local lGetScoreStep = {configData.llsmall,configData.llmiddle,configData.llbig,configData.llsuperbig}
		self:showRewardTipNode(function() 
			if self.isAuto then
				self:autoSendMsg()
			end
			self:bigWinEnd()
			if self._effSoundHandle then
				g_ExternalFun.stopEffect(self._effSoundHandle)
				self._effSoundHandle = nil
			end
		end,beishu,self.totalAward,lGetScoreStep)
    	--self:showBinWinPanel(index)
    end  
    return isHaveAnim
end
--超级旋转回调（播放速度动画）
...
function GameViewLayer:setBigwinConfig()
    local betScore = self.betCfg[self.betCfgIdx] * g_var(cmd).MAX_LINE
    self.bigwinConfig = {}
    ...
    self.bigwinConfig[4] = self.configData.llsuperbig * betScore
end
...
--奖励提示界面
function GameViewLayer:showRewardTipNode(_call, _rate, _nums,lGetScoreStep)
    ...
    data._rateNum = _rate
    data._nums = _nums
    data._lGetScoreStep = lGetScoreStep
    data._auto = self.isAuto
    local _rewardTipNode = RewardTipNode:create(data):addTo(self.m_csbNode, 100000)
    _rewardTipNode:setPosition(self.m_csbNode:getContentSize().width * 0.5, self.m_csbNode:getContentSize().height * 0.5)
    _rewardTipNode:setName("RewardTipNode")
end
 
function GameViewLayer:hideRewardTipNode()
    self.m_csbNode:removeChildByName("RewardTipNode")
end
