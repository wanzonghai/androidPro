...
function Game1ViewLayer:initUI( csbNode )
 ...
 --    Button_Stop:setVisible(false)
    ...
    function self:showGold(num,gold)
        ...
        self.allGold = gold--self.allGold+gold
        if self.allGold and self.allGold > 0 then
            local serverKind = G_GameFrame:getServerKind()
            self.m_textGetScore:setString(g_format:formatNumber(self.allGold,g_format.fType.standard,serverKind))  --更新总赢分
        else
            local serverKind = G_GameFrame:getServerKind()
            self.m_textGetScore:setString(g_format:formatNumber(self.allGold,g_format.fType.standard,serverKind))  --更新总赢分
        end
        ...
        self.nodeJs:runAction(cc.Sequence:create(cc.DelayTime:create(time),cc.CallFunc:create(function()
            self.txtJsNum:setVisible(false)
            self.nodePar:setVisible(false)
        end)))
    end
...
function Game1ViewLayer:onBtnRule()
    ExternalFun.playSoundEffect("sound-tiger-line-button.mp3")
    if self._scene.m_FreeTime > 0 then return end
    -- self._scene.m_bYafenIndex = #self._scene.m_lBetScore-1
    -- self._scene.m_lYaxian = 9
...
function Game1ViewLayer:game1BeginBefore()
    ...
    for i=1,5 do
        ...
        local moveby7 = cc.CallFunc:create(function()
                    self.m_fruitNode[i]:setPositionY(self.m_fruitNodePosY[i]+1890)--205*9)
                  end)
        local moveby8 = cc.MoveBy:create(0.6, cc.p(0, -1750))--[[-125*15+140 ))]]
        local moveby9 = cc.CallFunc:create(function()
                    self.m_fruitNode[i]:setPositionY(self.m_fruitNodePosY[i]+1890)--205*9)
                end)
        local moveby10 = cc.MoveBy:create(0.6, cc.p(0, -1750))--[[-125*15+140 ))]]
...
function Game1ViewLayer:game1Begin()
    ...
    for i=1,5 do
        ...
        self.m_fruitNode[i].bEndAction = false
        --local delay = cc.DelayTime:create(delayTime[i])
        local moveby6 = cc.MoveBy:create(0.5+delayTime[i], cc.p(0, -1750))--[[-125*15+140 ))]]
...
function Game1ViewLayer:showFreeWinNode(_nums)
    ...
    local youwin = sp.SkeletonAnimation:create(GameViewLayer.RES_PATH .. "spine/jiesuan.json",GameViewLayer.RES_PATH .. "spine/jiesuan.atlas", 1)
    youwin:setPosition(self._csbNode:getContentSize().width * 0.5, self._csbNode:getContentSize().height * 0.5+80)
    youwin:setAnimation(0, "animation2", true)      
    _freeWinNode:addChild(youwin,10)
    youwin:setScale(0.3)

    local scale1 = cc.ScaleTo:create(0.11,0.8)
    local scale2 = cc.ScaleTo:create(0.11,0.7)
    ...
    :setFntFile(GameViewLayer.RES_PATH .. "fonts/youwin.fnt")
    local serverKind = G_GameFrame:getServerKind()
    local formatMoney = g_format:formatNumber(_nums,g_format.fType.standard,serverKind)
    freeWinText:setString(formatMoney)
    freeWinText:setPosition(youwin:getContentSize().width/2+20,youwin:getContentSize().height/2-110)
    youwin:addChild(freeWinText,10)

    _freeWinNode:runAction(cc.Sequence:create(cc.DelayTime:create(2),cc.CallFunc:create(function()
