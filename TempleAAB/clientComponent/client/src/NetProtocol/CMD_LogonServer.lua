...
--破产补助 - 领取奖励  2104
login.CMD_MB_BaseEnsureTakeReward = {
	{t='dword',     k='dwUserID',},                         					--UserID	
	{t='tchar',     k='szDynamicPass',       s=G_NetLength.LEN_PASSWORD,},    	--动态密码
	{t='tchar',     k='szIP',          s=16,},                    		-- IP地址
	{t='tchar',     k='szMachineID',	s=33,},               -- 机器码 定长    						
}
...
login.CMD_tagPayChannel = {
	{t='dword',     k='dwChannelID',},   		-- 支付渠道ID              
	{t='tchar',     k='szChannelName',s=16},			--渠道名称,epay,dpay ...三方商家的名称
	{t='score',     k='lMinLimit'},						--[商城商品部分可以不用处理] 下限0表示无限(提现金额下限)
	{t='score',     k='lMaxLimit'},					--[商城商品部分可以不用处理] 上限0表示无限(提现金额上限)
	{t='bool',     k='boSupportWithdraw'},			--[商城商品部分可以不用处理] 此渠道是否支持提现
	{t='tchar',     k='szSupportBankCode',s=256},	--[商城商品部分可以不用处理] 支持的提现银行代码  CPF|PHONE|EMAIL|EVP
	{t='byte',     k='cbDefaultMask'}			--是否为默认支付/提现渠道 0x01表示为默认的支付渠道，0x02为默认提现渠道，0x03为两者是默认的
}
