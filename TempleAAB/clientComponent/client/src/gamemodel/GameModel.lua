...
local TipsChangeNote = appdf.req("client.src.UIManager.hall.TipsChangeNote")
local JumpVipDialog = appdf.req("client.src.UIManager.hall.subinterface.Dialog.JumpVipDialog")
...
function GameModel:onExit()
    self:hideTipsChangeNote()
    self:hideJumpVipDialog()
	self:KillGameClock()
    GlobalUserItem.bAutoConnect = true
...
--游戏通用，未充值切换筹码提示界面 显示
function GameModel:showTipsChangeNote(callback)
    local _tips_change_note = self:getChildByName("_tips_change_note")
    if _tips_change_note then
        return
     end

     local _callback = callback or function()
        self:onExitTable()
     end

     local _tips_change_note = TipsChangeNote:create({callback = _callback})
     _tips_change_note:setName("_tips_change_note")
     self:addChild(_tips_change_note) 
end

--游戏通用，未充值切换筹码提示界面 关闭
function GameModel:hideTipsChangeNote()
    local _tips_change_note = self:getChildByName("_tips_change_note")
    if _tips_change_note ~= nil then
        _tips_change_note:removeFromParent(true)
    end
end

--游戏通用，钱不足提示界面 显示
function GameModel:showJumpVipDialog(callback)
    local _callback = callback or function()
        self:onExitTable()
     end
    --您可以通过以下两种方式获取金币
...
--游戏通用，钱不足提示界面 关闭
function GameModel:hideJumpVipDialog()
    local parent = cc.Director:getInstance():getRunningScene() 
    local _jump_vip_dialog = parent:getChildByName("_jump_vip_dialog")
    if _jump_vip_dialog ~= nil then
        _jump_vip_dialog:removeFromParent(true)
    end
end
