...
function HallVIPHelpLayer:ctor(args)
    ...
    self._vipHelperConfigData = args
...
function HallVIPHelpLayer:onGetGrowLevelConfig(data)
    ...
    for i, v in ipairs(data) do
        ...
        local Text_0 = item:getChildByName("Text_0")  
        ...
        local Text_6 = item:getChildByName("Text_6")   
        if i%2 ~= 0 then
            itemBG:setVisible(false)
        end
        Text_0:setString((self._vipHelperConfigData and self._vipHelperConfigData[i] or 0) / 10 .."%")
        ...
        if v.cbGrowLevel == GlobalUserItem.VIPLevel then
            ...
            Text_6:setTextColor(cc.c3b(234,130,7))
        end
        ...
        local totalScore = v.llScoreFirst + v.llScoreScecond
        local base_ensure = g_format:formatNumber(totalScore,g_format.fType.abbreviation,g_format.currencyType.GOLD)
        Text_6:setString(base_ensure)  -- 破产补助
