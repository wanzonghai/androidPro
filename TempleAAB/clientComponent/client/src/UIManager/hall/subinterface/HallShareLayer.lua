...
function HallShareLayer:shareStr(index)
   ...
   local shareUrl = self.config.szShareUrl.."/?t=2&d="..userStr
   --2023-08-06-22:38 FIX
   --[[
    local str = string.urlencode(shareUrl)
    g_MultiPlatform:getInstance():mobShare(platformStr,str,self.config.szShareTips,"","")  
    --]]
    ...
    if device.platform == "ios" then
        local encodeShareString = string.urlencode(shareUrl)
        local url = encodeShareString
        if platformStr == "WhatsApp" then
            url = "whatsapp://send?text="..encodeShareString
        elseif platformStr == "Messenger" then
            url = "fb-messenger-api://?message="..encodeShareString
        elseif platformStr == "Telegram" then
            url = "tg://msg?text="..encodeShareString
        end
        OSUtil.openURL(url)
    else
...
function HallShareLayer:onPhoneShareCallback(data)
    if data == nil then
        ...
        G_ServerMgr:C2S_GetShareReward(type)
    end
end
...
function HallShareLayer:setQrCode()
    ...
    --local shareUrl = self.config.szShareUrl.."/spd_"..userStr..'.shtml'
    local shareUrl = self.config.szShareUrl.."/?t=2&d="..userStr
    --local strUrl = string.urlencode(shareUrl)
