...
function CashOutLayer:ctor(args)
   ...
   self.inputName = self.PanelName:getChildByName("inputPsd1"):convertToEditBox()
    ...
    G_event:AddNotifyEvent(G_eventDef.CMD_MB_PlaceWithdrawOrderResult,handler(self,self.onPlaceWithdrawOrderResult))   --提现结果返回
...
--     if GlobalUserItem.lUserScore >= self.goldList[i].lAwardValue then
function CashOutLayer:checkInputScore()
    ...
    if value * 100 > GlobalUserItem.lUserScore then
        value = math.modf(GlobalUserItem.lUserScore/100)
    end
...
function CashOutLayer:updateUserGold()
    ...
    if self.cashOutInfo then
        self.PanelLeft:getChildByName("text_total"):setString(g_format:formatNumber(GlobalUserItem.lUserScore, g_format.fType.standard))
        if self.cashOutInfo.lCurrentBetScore >= self.cashOutInfo.lRequireBetScore then
            self.PanelLeft:getChildByName("text_out"):setString(g_format:formatNumber(GlobalUserItem.lUserScore, g_format.fType.standard))
        else
...
function CashOutLayer:updateUserInsure()
    print("CashOutLayer:updateUserInsure", GlobalUserItem.lUserScore)
    self:updateUserGold()
    if self.cashOutInfo then
        --额度按钮
        for i=1,m_btn_num do
...
function CashOutLayer:onConfirmClicked()
    if self.cashOutInfo and self.cashOutInfo.lRequireBetScore then
        if self.cashOutInfo.lCurrentBetScore >= self.cashOutInfo.lRequireBetScore then
            if GlobalUserItem.lUserScore < self.min_value then --金币存入银行才能提现，最低50
                local txt = "O seu saldo é insuficiente, com um valor mínimo de R$"..self.min_value.."."
...
function CashOutLayer:btnConfirmClick()    
    ...
    if self.cashOutInfo then
        local accountTypeName = self.cashOutInfo.accountTypeTb[self.accountIdx]        
        if accountTypeName then
            ...
            local pType = CASH_ACCOUNT_CHECK[self.accountIdx].name 
            pType = string.lower(string.gsub(pType," ",""))
            local pExtends = {
                WalletType = pType,--钱包类型
                WalletNumber= inputAcount,--钱包号码
                WalletHolder= inputName,--钱包持有人
                IDNumber= inputCpf,
                TokenApp= GlobalData.AppToken,--Adjust 应用token
                TokenCZ= "",--充值事件
                TokenSC= "",--首充事件
                TokenTX= GlobalData.WithdrawToken,--提现事件
            ...
            G_ServerMgr:C2S_CMD_MB_PlaceWithdrawOrder(params)
            showNetLoading()
        end
    end    
end

function CashOutLayer:onPlaceWithdrawOrderResult(pResult)
    if pResult and pResult.code and pResult.code == 0 then        
        ...
        local pString = "Ocorreu uma exceção durante o saque, por favor tente novamente!"
