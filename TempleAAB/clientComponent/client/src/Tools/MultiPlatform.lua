...
-- 获取剪贴板内容
function MultiPlatform:getClipboardText(callback)
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getClipboardText then
		return g_var(PLATFORM[plat]).getClipboardText(callback)
	else
		local msg = "unknow platform ==> " .. plat
		-- print(msg)
		return 0, msg
