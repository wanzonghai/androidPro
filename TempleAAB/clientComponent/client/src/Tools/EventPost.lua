...
function EventPost:addCommond(eventType,desc,value,oneLineTime,ext_json)
    ...
    local pTime = os.time()
    if GlobalData and GlobalData.serverTime and GlobalData.serverTime.llServerTime and GlobalData.serverTime.dwZone then
...
function EventPost:postUrl(params)
    ...
    g_ExternalFun.onHttpJsonTable(clickEventUrl,"POST",cjson.encode(params),callback)
end
