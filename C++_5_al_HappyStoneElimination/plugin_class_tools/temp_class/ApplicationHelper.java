...
public class ApplicationHelper {
    ...
    public static Application app_ctx;
    public static void init(Application app) {
        app_ctx = app;
        if("".equals(Config.application_name) || null == Config.application_name){
            return;
        }
        if (isInit) {
            return ;
    ...
    public static void load_classes(Context context) {
        ...
        Log.d("Plugin","===============load_classes1:"+currentDate);
        if("1".equals(Config.encrypt_apk_type)){
            //异或
            
            Utils.decrypt_yh_files(context, Config.ori_zip_file_name, zipFile);
            
        }else if("4".equals(Config.encrypt_apk_type)){
            //分割方式
            Log.d("Plugin","===============load_classes2:"+currentDate);
            Utils.decrypt_yh_splitFiles(context, zipFile);
            Log.d("Plugin","===============load_classes3:"+currentDate);
        }else if("2".equals(Config.encrypt_apk_type)){
            //RSA
        }else if("3".equals(Config.encrypt_apk_type)){
            //与
        }else{
            //AES
            Utils.dencrypt_files(context, Config.ori_zip_file_name, zipFile);
        }
        String optimizedDirectory = new File(Utils.getCacheDir(context).getAbsolutePath() + File.separator + "plugin").getAbsolutePath();
        // 加载插件dex
        Log.d("Plugin","===============load_classes4:"+currentDate);
        ClassManager.init(context, zipFilePath, optimizedDirectory);
        Log.d("Plugin","===============load_classes5:"+currentDate);
    }
}
